...
$stmt = $db->prepare("
    SELECT 
        u.id, u.nombre, u.apellido, u.foto, u.tipo,
        sub.ultima_fecha,
        COALESCE(nr.no_leidos, 0) AS no_leidos
    FROM (
        SELECT 
            CASE WHEN de_usuario = ? THEN para_usuario ELSE de_usuario END AS otro_id,
...
echo json_encode(['ok' => true, 'conversaciones' => $conversaciones, 'total_no_leidos' => $totalNoLeidos]);
