...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'subir_evidencia') {
        ...
        if ($totalActual >= $limite) {
            echo json_encode(['ok'=>false,'msg'=>$tieneSelvaVerde
        ...
        if ($tipoMedia === 'video_url') {
            ...
            if (!$urlVideo) { echo json_encode(['ok'=>false,'msg'=>'URL de video requerida']); exit; }
        ...
        if (!$esFoto && !$esVideo) {
            echo json_encode(['ok'=>false,'msg'=>'Formato no permitido. Usa JPG, PNG, MP4 o MOV.']); exit;
        }
        $maxSize = $esVideo ? 50*1024*1024 : 5*1024*1024; // 50MB video, 5MB foto
        if ($_FILES['archivo']['size'] > $maxSize) {
            echo json_encode(['ok'=>false,'msg'=>$esVideo ? 'Máximo 50 MB para videos.' : 'Máximo 5 MB para fotos.']); exit;
        }
    ...
    if ($action === 'eliminar_evidencia') {
        ...
        if (!$gid) { echo json_encode(['ok'=>false,'msg'=>'ID inválido']); exit; }
    ...
    if ($action === 'registrar_vista') {
        ...
        if ($uid && $uid !== $usuario['id']) {
            ...
            } catch(Exception $e) { echo json_encode(['ok'=>false]); exit; }
        }
...
} catch(Exception $e) { $galeriaItems = []; $galeriaTotal = 0; }
...
} catch(Exception $e){ $extras = []; }

// Detectar subtipo desde extras
$subTipo = ''; // dj, fotografo, negocio_cc, negocio_emp, etc.
if (!empty($extras['tipo_negocio_reg'])) $subTipo = $extras['tipo_negocio_reg'] === 'cc' ? 'negocio_cc' : 'negocio_emp';
...
} catch(Exception $e){ $vistasTotal = 0; $vistas7dias = 0; }
...
function calcPct(array $usuario, array $talento, string $tipo, array $extras): int {
    ...
    if ($tipo === 'candidato') {
        if (!empty($talento['profesion'])) $pct = min(100,$pct+15);
        if (!empty($talento['bio']))       $pct = min(100,$pct+10);
        if (!empty($talento['skills']))    $pct = min(100,$pct+5);
    } elseif ($tipo === 'empresa') {
        if (!empty($extras['sector']??'')) $pct = min(100,$pct+15);
        if (!empty($extras['rep_legal']??'')) $pct = min(100,$pct+10);
    }
    if (!empty($usuario['foto'])) $pct = min(100,$pct+10);
    return $pct;
}
...
if ($subTipo === 'negocio_cc') $tc['label'] = '🏬 C.C. El Caraño';
if ($subTipo === 'servicio')   { $tc['label']='🎧 Servicios'; $tc['deco']='🎧'; $tc['border']='var(--a3)'; $tc['chip_color']='var(--acrem)'; }
?>
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <style>
    :root{
      ...
      --a1:#b38000;--a2:#d4a017;--a3:#f5c800;--a4:#ffd94d;--acrem:#fff3b0;
      --r1:#002880;--r2:#0039a6;--r3:#1a56db;--r4:#5b8eff;--rcielo:#b8d4ff;
      --bg:#060e07;--bg2:#0c1a0e;--bg3:#111f13;
      --card:rgba(15,28,17,.9);--borde:rgba(255,255,255,.08);
      --ink:rgba(255,255,255,.88);--ink2:rgba(255,255,255,.55);--ink3:rgba(255,255,255,.32);
    }
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ...
    /* ── FRANJA TRICOLOR TOP ── */
    .franja-top{position:fixed;top:0;left:0;right:0;height:3px;display:flex;z-index:999}
    .franja-top span{flex:1}
    .franja-top span:nth-child(1){background:var(--v3)}
    .franja-top span:nth-child(2){background:var(--a3)}
    .franja-top span:nth-child(3){background:var(--r3)}

    /* ── NAVBAR ── */
    .navbar{position:sticky;top:3px;z-index:200;background:rgba(4,14,7,.95);backdrop-filter:blur(20px);border-bottom:1px solid var(--borde);display:flex;align-items:center;justify-content:space-between;padding:0 36px;height:56px}
    .nav-marca{display:flex;align-items:center;gap:10px;text-decoration:none}
    .nav-marca img{width:28px;filter:drop-shadow(0 2px 6px rgba(163,240,181,.3))}
    .nav-marca-txt{font-family:'Fraunces',serif;font-size:18px;color:white}
    .nav-marca-txt em{color:var(--vlima);font-style:normal}
    .nav-links{display:flex;align-items:center;gap:2px}
    .nl{display:flex;align-items:center;gap:6px;padding:7px 12px;border-radius:10px;color:var(--ink3);text-decoration:none;font-size:13px;font-weight:600;transition:all .2s;position:relative;white-space:nowrap}
    .nl:hover{background:rgba(255,255,255,.07);color:var(--ink)}
    .nl.on{background:rgba(163,240,181,.1);color:var(--vlima)}
    .nl-dot{position:absolute;top:5px;right:5px;width:7px;height:7px;border-radius:50%;background:#e74c3c;border:1.5px solid #060e07}
    .nav-right{display:flex;align-items:center;gap:10px}
    .nav-nombre{font-size:13px;font-weight:700;color:var(--ink2);max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .nav-av{width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,var(--v2),var(--vlima));display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:900;color:white;cursor:pointer;border:2px solid rgba(163,240,181,.2);overflow:hidden;flex-shrink:0;transition:border-color .2s}
    .nav-av:hover{border-color:var(--vlima)}
    .nav-notif{position:relative;width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,.05);border:1px solid var(--borde);display:flex;align-items:center;justify-content:center;font-size:16px;cursor:pointer;transition:background .2s;flex-shrink:0}
    .nav-notif:hover{background:rgba(255,255,255,.1)}
    .notif-dot{position:absolute;top:4px;right:4px;width:8px;height:8px;border-radius:50%;background:#e74c3c;border:1.5px solid #060e07;animation:pulse-dot 1.5s infinite;display:none}
    @keyframes pulse-dot{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.3);opacity:.7}}
    .notif-panel{position:absolute;top:calc(100% + 10px);right:0;width:290px;background:#0c1a0e;border:1px solid rgba(255,255,255,.12);border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,.5);z-index:9999;overflow:hidden;display:none}
    .notif-panel.open{display:block}
    .notif-head{padding:12px 16px;font-size:11px;font-weight:800;color:var(--vlima);border-bottom:1px solid var(--borde);text-transform:uppercase;letter-spacing:.06em}
    .notif-item{padding:12px 16px;display:flex;gap:10px;align-items:flex-start;border-bottom:1px solid rgba(255,255,255,.04);font-size:13px;color:var(--ink2)}
    .notif-item:last-child{border-bottom:none}
    .notif-ico{font-size:18px;flex-shrink:0;margin-top:1px}
    .notif-sub{font-size:11px;color:var(--ink3);margin-top:2px}
    .notif-empty{padding:18px 16px;text-align:center;color:var(--ink3);font-size:13px}
    .nav-salir{padding:6px 12px;border-radius:10px;background:rgba(255,255,255,.05);border:1px solid var(--borde);color:var(--ink3);font-size:12px;font-weight:700;text-decoration:none;transition:all .2s}
    .nav-salir:hover{background:rgba(231,76,60,.15);color:#ff8080;border-color:rgba(231,76,60,.25)}
    ...
    .hero{
      ...
      padding:40px 36px 0;position:relative;overflow:hidden;
    }
    .hero::after{content:'';position:absolute;bottom:0;left:0;right:0;height:36px;background:var(--bg);border-radius:36px 36px 0 0}
    /* Borde inferior tipo de usuario */
    .hero-tipo-borde{position:absolute;bottom:35px;left:36px;right:36px;height:2px;background:linear-gradient(to right,<?= $tc['border'] ?>,transparent)}
    .hero-inner{max-width:1200px;margin:0 auto;display:grid;grid-template-columns:auto 1fr auto;align-items:end;gap:28px;padding-bottom:48px;position:relative;z-index:2}
    .hero-av{
      ...
      flex-shrink:0;cursor:pointer;overflow:hidden;transition:all .25s;
    }
    .hero-av:hover{border-color:var(--vlima);transform:scale(1.03)}
    .hero-chips{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px}
    .hchip{display:inline-flex;align-items:center;gap:4px;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:700;letter-spacing:.3px}
    .hc-tipo{background:<?= $tc['chip_bg'] ?>;color:<?= $tc['chip_color'] ?>;border:1px solid <?= $tc['border'] ?>40}
    .hc-v{background:rgba(45,138,80,.2);color:#6dea8f;border:1px solid rgba(45,138,80,.3)}
    .hc-p{background:rgba(200,134,10,.2);color:var(--a4);border:1px solid rgba(200,134,10,.3)}
    .hc-top{background:rgba(239,68,68,.2);color:#fca5a5;border:1px solid rgba(239,68,68,.3)}
    .hc-dest{background:rgba(139,92,246,.2);color:#c4b5fd;border:1px solid rgba(139,92,246,.3)}
    .hero-nombre{font-family:'Fraunces',serif;font-size:clamp(28px,3vw,40px);color:white;line-height:1.1;margin-bottom:6px}
    .hero-nombre em{color:var(--vlima);font-style:normal}
    .hero-sub{font-size:14px;color:var(--ink2);font-weight:500}
    .hero-sub strong{color:var(--ink)}
    .hero-stats{display:flex;gap:20px;padding-bottom:6px}
    .hs{text-align:center;min-width:70px}
    .hs-val{font-family:'Fraunces',serif;font-size:24px;font-weight:900;color:white;line-height:1}
    .hs-lab{font-size:10px;color:var(--ink3);text-transform:uppercase;letter-spacing:.7px;margin-top:3px;font-weight:600}
    .hero-deco{position:absolute;right:40px;top:20px;font-size:100px;opacity:.04;user-select:none;pointer-events:none}

    /* ── CONTENIDO ── */
    .contenido{max-width:1200px;margin:0 auto;padding:28px 36px 80px}

    /* ── ALERTAS ── */
    .alerta{border-radius:16px;padding:14px 18px;display:flex;align-items:center;gap:14px;margin-bottom:22px;border:1.5px solid}
    .alerta.ap{background:rgba(245,200,0,.07);border-color:rgba(245,200,0,.25)}
    .alerta.ar{background:rgba(239,68,68,.07);border-color:rgba(239,68,68,.2)}
    .alerta.as{background:rgba(255,255,255,.04);border-color:var(--borde)}
    .alerta.av{background:rgba(39,168,85,.07);border-color:rgba(39,168,85,.2)}
    .a-ico{font-size:22px;flex-shrink:0}
    .a-txt{flex:1}
    .a-txt strong{display:block;font-size:14px;font-weight:800;margin-bottom:2px}
    .alerta.ap .a-txt strong{color:var(--a4)}
    .alerta.ar .a-txt strong{color:#ff8080}
    .alerta.as .a-txt strong{color:var(--ink)}
    .alerta.av .a-txt strong{color:var(--vlima)}
    .a-txt span{font-size:12px;color:var(--ink2);line-height:1.5}
    .a-btn{flex-shrink:0;padding:7px 16px;border-radius:20px;border:none;cursor:pointer;font-size:12px;font-weight:800;font-family:'DM Sans',sans-serif;text-decoration:none}
    .alerta.ap .a-btn{background:rgba(245,200,0,.15);color:var(--a4)}
    .alerta.ar .a-btn,.alerta.as .a-btn{background:var(--v3);color:white}
    ...
    /* ── CARD ── */
    .card{background:var(--card);border-radius:20px;border:1px solid var(--borde);box-shadow:0 2px 16px rgba(0,0,0,.3);overflow:hidden;transition:box-shadow .25s,transform .25s}
    .card:hover{box-shadow:0 8px 32px rgba(0,0,0,.4);transform:translateY(-2px)}
    .card-pad{padding:22px}

    /* ── MINI MÉTRICAS ── */
    .mini{padding:22px;display:flex;flex-direction:column;justify-content:space-between;min-height:120px}
    .m-ico{width:40px;height:40px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:18px;margin-bottom:14px}
    .ig{background:rgba(39,168,85,.18)} .ia{background:rgba(26,86,219,.18)} .im{background:rgba(139,92,246,.18)} .io{background:rgba(245,200,0,.18)}
    .m-val{font-family:'Fraunces',serif;font-size:26px;font-weight:900;color:white;line-height:1}
    .m-lab{font-size:11px;color:var(--ink3);margin-top:4px;font-weight:700;text-transform:uppercase;letter-spacing:.4px}
    .m-sub{font-size:10px;color:var(--v4);margin-top:3px;font-weight:800}

    /* ── ACCIONES RÁPIDAS ── */
    .ca-tit{font-size:11px;font-weight:800;color:var(--ink3);text-transform:uppercase;letter-spacing:.7px;margin-bottom:16px;padding:22px 22px 0}
    .ac-row{display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:8px;padding:0 22px 22px}
    .ac{display:flex;flex-direction:column;align-items:center;gap:6px;padding:14px 8px;border-radius:16px;background:rgba(255,255,255,.04);border:1px solid var(--borde);text-decoration:none;transition:all .22s;position:relative}
    .ac:hover{background:rgba(163,240,181,.07);border-color:rgba(163,240,181,.2);transform:translateY(-2px)}
    .ac-ico{font-size:22px}
    .ac-tit{font-size:12px;font-weight:700;color:var(--ink);text-align:center}
    ...
    .ce-head{display:flex;align-items:center;justify-content:space-between;padding:22px 22px 14px}
    .ce-tit{font-size:14px;font-weight:800;color:var(--ink)}
    .ce-ver{font-size:12px;font-weight:700;color:var(--vlima);text-decoration:none}
    .ce-ver:hover{text-decoration:underline}
    .ce-list{padding:0 22px 22px;display:flex;flex-direction:column;gap:8px}
    .ce-item{display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:14px;background:rgba(255,255,255,.04);border:1px solid var(--borde);cursor:pointer;transition:all .2s}
    .ce-item:hover{background:rgba(163,240,181,.07);border-color:rgba(163,240,181,.2)}
    .ce-ico{font-size:22px;flex-shrink:0}
    .ce-info{flex:1;min-width:0}
    .ce-nom{font-size:13px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--ink)}
    .ce-emp{font-size:12px;color:var(--ink2)}
    .ce-met{font-size:11px;color:var(--vlima);font-weight:600;margin-top:2px}
    .ce-badge{font-size:11px;font-weight:700;padding:3px 9px;border-radius:10px;background:rgba(163,240,181,.1);color:var(--vlima);white-space:nowrap;flex-shrink:0;border:1px solid rgba(163,240,181,.15)}

    /* ── HISTORIAL VACANTES ── */
    .hist-item{display:flex;align-items:center;gap:12px;padding:10px 14px;border-radius:12px;background:rgba(255,255,255,.04);border:1px solid var(--borde)}
    .hdot{width:9px;height:9px;border-radius:50%;flex-shrink:0}
    .hdot.act{background:#22c55e} .hdot.cer{background:#94a3b8}
    .hnom{font-size:13px;font-weight:700;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .hmeta{font-size:11px;color:var(--ink3);margin-top:2px}
    .hfecha{font-size:11px;color:var(--ink3);flex-shrink:0;white-space:nowrap}

    /* ── PERFIL CARD ── */
    .cp-head{display:flex;flex-direction:column;align-items:center;text-align:center;padding:22px 22px 16px;border-bottom:1px solid var(--borde)}
    .cp-av{width:76px;height:76px;border-radius:20px;background:linear-gradient(135deg,var(--v1),var(--v3));display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:900;color:white;margin-bottom:12px;cursor:pointer;border:2px solid rgba(163,240,181,.15);overflow:hidden;transition:all .2s}
    .cp-av:hover{border-color:var(--vlima)}
    .cp-nom{font-size:16px;font-weight:900;color:var(--ink);margin-bottom:3px}
    .cp-pro{font-size:13px;color:var(--vlima);font-weight:600}
    .cp-body{padding:14px 22px;display:flex;flex-direction:column;gap:8px}
    .cp-fil{display:flex;align-items:center;gap:8px;font-size:13px;color:var(--ink2)}
    .cp-ico{font-size:15px;flex-shrink:0}

    /* Visibilidad toggle */
    .vis-row{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(255,255,255,.04);border-radius:12px;border:1px solid var(--borde);margin:4px 0}
    .vis-lab{font-size:12px;font-weight:700;color:var(--ink2)}
    .vis-sub{font-size:10px;color:var(--ink3);margin-top:1px}
    .tog{position:relative;width:42px;height:22px;cursor:pointer;flex-shrink:0}
    .tog input{opacity:0;width:0;height:0}
    .tog-sl{position:absolute;inset:0;border-radius:11px;background:rgba(255,255,255,.15);transition:.3s}
    .tog-sl::before{content:'';position:absolute;width:16px;height:16px;left:3px;top:3px;border-radius:50%;background:white;transition:.3s;box-shadow:0 1px 4px rgba(0,0,0,.3)}
    .tog input:checked + .tog-sl{background:var(--v3)}
    .tog input:checked + .tog-sl::before{transform:translateX(20px)}
    .pv-chip{font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;display:inline-block}
    .pv-chip.ok{background:rgba(39,168,85,.15);color:var(--vlima);border:1px solid rgba(39,168,85,.25)}
    .pv-chip.off{background:rgba(245,200,0,.12);color:var(--a4);border:1px solid rgba(245,200,0,.2)}

    /* Progreso */
    .prog-w{padding:0 22px 8px}
    .prog-h{display:flex;justify-content:space-between;font-size:11px;font-weight:700;margin-bottom:6px;color:var(--ink3)}
    .prog-t{height:5px;background:rgba(255,255,255,.08);border-radius:4px;overflow:hidden}
    .prog-f{height:100%;background:linear-gradient(90deg,var(--v2),var(--vlima));border-radius:4px;transition:width 1s ease}

    /* Botones */
    .btn-edit{margin:14px 22px 22px;padding:11px;border-radius:14px;background:linear-gradient(135deg,var(--v1),var(--v3));color:white;border:none;font-size:13px;font-weight:800;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s;box-shadow:0 4px 12px rgba(39,168,85,.3);display:block;width:calc(100% - 44px);text-align:center;text-decoration:none}
    .btn-edit:hover{transform:translateY(-1px);box-shadow:0 6px 18px rgba(39,168,85,.4)}
    .btn-sec{margin:0 22px 16px;padding:11px;border-radius:14px;background:transparent;color:var(--vlima);border:1.5px solid rgba(163,240,181,.25);font-size:13px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s;display:block;width:calc(100% - 44px);text-align:center;text-decoration:none}
    .btn-sec:hover{background:rgba(163,240,181,.07);border-color:rgba(163,240,181,.4)}

    /* Actividad */
    .cact-tit{padding:22px 22px 14px;font-size:11px;font-weight:800;color:var(--ink3);text-transform:uppercase;letter-spacing:.7px}
    .act-list{padding:0 22px 22px;display:flex;flex-direction:column;gap:8px}
    .act-it{display:flex;align-items:center;gap:12px}
    .act-pt{width:32px;height:32px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0}
    .pv{background:rgba(39,168,85,.15)} .pa{background:rgba(26,86,219,.15)} .po{background:rgba(245,200,0,.12)}
    .act-n{font-size:13px;font-weight:600;color:var(--ink)}
    .act-f{font-size:11px;color:var(--ink3);margin-top:1px}

    /* Bandera mini en dashboard */
    .bandera-dash{width:52px;height:32px;border-radius:7px;overflow:hidden;display:flex;flex-direction:column;border:1.5px solid rgba(255,255,255,.15);box-shadow:0 3px 10px rgba(0,0,0,.4);flex-shrink:0;position:relative}
    .banda{flex:1}
    .banda-v{background:var(--v2)} .banda-a{background:var(--a3)} .banda-r{background:var(--r2)}
    .bandera-dash::after{content:'';position:absolute;inset:0;background:linear-gradient(120deg,rgba(255,255,255,.15) 0%,transparent 60%);border-radius:5px;pointer-events:none}

    /* ── MODAL EDITAR ── */
    .modal-ov{display:none;position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:500;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(4px)}
    .modal-ov.open{display:flex}
    .modal-box{background:#0c1a0e;border:1px solid rgba(163,240,181,.15);border-radius:24px;max-width:560px;width:100%;box-shadow:0 30px 80px rgba(0,0,0,.6);animation:fadeUp .3s ease;max-height:90vh;overflow-y:auto;position:relative}
    ...
    .mtit{font-family:'Fraunces',serif;font-size:22px;font-weight:700;margin-bottom:6px;color:var(--ink)}
    ...
    .mmsg.success{background:rgba(163,240,181,.12);border:1px solid rgba(163,240,181,.25);color:var(--vlima)}
    .mmsg.error{background:rgba(255,80,80,.12);border:1px solid rgba(255,80,80,.25);color:#ff9a9a}
    ...
    .mgr label{font-size:11px;font-weight:700;color:var(--ink3);text-transform:uppercase;letter-spacing:.5px}
    .mgr input,.mgr select,.mgr textarea{border:1.5px solid var(--borde);border-radius:12px;padding:10px 12px;font-size:13px;font-family:'DM Sans',sans-serif;color:var(--ink);background:rgba(255,255,255,.05);transition:border-color .2s;outline:none;resize:none}
    .mgr input:focus,.mgr select:focus,.mgr textarea:focus{border-color:var(--v3);background:rgba(39,168,85,.06)}
    .mgr select option{background:#0c1a0e;color:var(--ink)}
    .btn-save{width:100%;padding:13px;border-radius:14px;background:linear-gradient(135deg,var(--v1),var(--v3));color:white;border:none;font-size:14px;font-weight:900;cursor:pointer;font-family:'DM Sans',sans-serif;margin-top:18px;box-shadow:0 4px 14px rgba(39,168,85,.3);transition:all .2s}
    .btn-save:hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(39,168,85,.4)}
    .btn-save:disabled{opacity:.5;cursor:not-allowed;transform:none}

    /* Crop modal */
    .crop-modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.8);z-index:99999;align-items:center;justify-content:center;padding:20px}
    .crop-inner{background:#0c1a0e;border:1px solid var(--borde);border-radius:20px;padding:24px;max-width:420px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.5)}
    /* ── SECCIONES DE PERFIL (Educación, Certificaciones, Aptitudes) ── */
    .psec{background:var(--card);border:1px solid var(--borde);border-radius:20px;overflow:hidden;margin-top:20px;transition:box-shadow .25s}
    .psec:hover{box-shadow:0 8px 32px rgba(0,0,0,.4)}
    .psec-head{display:flex;align-items:center;justify-content:space-between;padding:18px 22px 0}
    .psec-tit{font-family:'Fraunces',serif;font-size:16px;font-weight:700;color:var(--ink)}
    .psec-btns{display:flex;gap:6px}
    .psec-btn{background:none;border:none;color:var(--ink3);font-size:18px;cursor:pointer;padding:4px;border-radius:8px;line-height:1;transition:all .2s}
    .psec-btn:hover{background:rgba(255,255,255,.08);color:var(--ink)}
    .psec-list{padding:14px 22px 8px}
    .psec-item{display:flex;align-items:flex-start;gap:14px;padding:14px 0;border-bottom:1px solid rgba(255,255,255,.05);position:relative}
    .psec-item:last-child{border-bottom:none}
    .psec-logo{width:44px;height:44px;border-radius:12px;background:rgba(255,255,255,.06);border:1px solid var(--borde);display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;overflow:hidden}
    .psec-logo img{width:100%;height:100%;object-fit:cover;border-radius:11px}
    .psec-body{flex:1;min-width:0}
    .psec-nom{font-size:14px;font-weight:800;color:var(--ink);line-height:1.3}
    .psec-sub{font-size:13px;color:var(--ink2);margin-top:2px}
    .psec-meta{font-size:12px;color:var(--ink3);margin-top:3px}
    .psec-credencial{display:inline-flex;align-items:center;gap:5px;margin-top:8px;padding:5px 12px;border:1px solid var(--borde);border-radius:20px;font-size:12px;font-weight:600;color:var(--ink2);background:rgba(255,255,255,.04);cursor:pointer;transition:all .2s;text-decoration:none}
    .psec-credencial:hover{border-color:var(--vlima);color:var(--vlima);background:rgba(163,240,181,.06)}
    .psec-archivo{display:flex;align-items:center;gap:10px;margin-top:8px;padding:9px 12px;border:1px solid var(--borde);border-radius:12px;background:rgba(255,255,255,.03);cursor:pointer;transition:background .2s;text-decoration:none}
    .psec-archivo:hover{background:rgba(255,255,255,.07)}
    .psec-arch-thumb{width:44px;height:40px;background:rgba(26,86,219,.15);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0}
    .psec-arch-name{font-size:12px;color:var(--ink2);font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .psec-item-del{position:absolute;top:14px;right:0;background:none;border:none;color:var(--ink3);font-size:14px;cursor:pointer;padding:4px;border-radius:6px;opacity:0;transition:all .2s}
    .psec-item:hover .psec-item-del{opacity:1}
    .psec-item-del:hover{color:#ff6b6b;background:rgba(255,107,107,.1)}
    .psec-ver-mas{display:flex;align-items:center;justify-content:center;gap:5px;padding:12px 22px;border-top:1px solid rgba(255,255,255,.05);font-size:13px;font-weight:700;color:var(--ink3);cursor:pointer;transition:color .2s;background:none;border-left:none;border-right:none;border-bottom:none;width:100%;font-family:'DM Sans',sans-serif}
    .psec-ver-mas:hover{color:var(--vlima)}
    /* Aptitudes */
    .apt-grupo{margin-bottom:16px}
    .apt-grupo:last-child{margin-bottom:0}
    .apt-nom{font-size:14px;font-weight:800;color:var(--ink);margin-bottom:6px}
    .apt-items{display:flex;flex-wrap:wrap;gap:6px}
    .apt-chip{display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:12px;background:rgba(255,255,255,.05);border:1px solid var(--borde);font-size:12px;font-weight:600;color:var(--ink2);cursor:default}
    .apt-chip-ico{font-size:14px}
    /* ── MODAL HOJA DE VIDA + PUBLICAR VACANTE ── */
    .hoja-modal-box{background:#0c1a0e;border:1px solid rgba(163,240,181,.15);border-radius:24px;max-width:700px;width:100%;box-shadow:0 30px 80px rgba(0,0,0,.6);animation:fadeUp .3s ease;max-height:92vh;overflow-y:auto;position:relative}
    .hoja-sec{font-size:11px;font-weight:800;color:var(--v4);text-transform:uppercase;letter-spacing:.8px;margin:20px 0 10px;display:flex;align-items:center;gap:8px}
    .hoja-sec-num{width:20px;height:20px;border-radius:50%;background:linear-gradient(135deg,var(--v2),var(--v3));display:flex;align-items:center;justify-content:center;font-size:9px;color:#fff;font-weight:900;flex-shrink:0}
    .hoja-divider{height:1px;background:rgba(255,255,255,.07);margin:16px 0}
    .hoja-fila{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:10px}
    @media(max-width:600px){.hoja-fila{grid-template-columns:1fr}}
    .hoja-gr{display:flex;flex-direction:column;gap:5px}.hoja-gr.full{grid-column:1/-1}
    .hoja-gr label{font-size:11px;font-weight:700;color:rgba(255,255,255,.38);text-transform:uppercase;letter-spacing:.6px}
    .hoja-gr input,.hoja-gr select,.hoja-gr textarea{width:100%;padding:11px 13px;background:rgba(255,255,255,.05);border:1.5px solid rgba(255,255,255,.1);border-radius:13px;color:#fff;font-size:13px;font-family:'DM Sans',sans-serif;outline:none;transition:border-color .2s,background .2s,box-shadow .2s;resize:none}
    .hoja-gr input:focus,.hoja-gr select:focus,.hoja-gr textarea:focus{border-color:var(--v3);background:rgba(39,168,85,.07);box-shadow:0 0 0 3px rgba(39,168,85,.1)}
    .hoja-gr input::placeholder,.hoja-gr textarea::placeholder{color:rgba(255,255,255,.18)}
    .hoja-gr select option{background:#0c1f10;color:#fff}
    .hoja-item-card{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.09);border-radius:14px;padding:14px 16px;margin-bottom:10px;position:relative;animation:fadeUp .3s ease both}
    .hoja-item-rm{position:absolute;top:10px;right:12px;background:rgba(255,80,80,.15);border:none;color:#ff7a7a;width:26px;height:26px;border-radius:50%;font-size:12px;cursor:pointer;display:flex;align-items:center;justify-content:center}
    .hoja-item-rm:hover{background:rgba(255,80,80,.3)}
    .hoja-btn-add{width:100%;padding:10px;background:rgba(39,168,85,.1);border:1.5px dashed rgba(39,168,85,.3);color:var(--v4);border-radius:12px;font-size:13px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer;transition:all .25s}
    .hoja-btn-add:hover{background:rgba(39,168,85,.18);border-color:var(--v3)}
    .hoja-progress-track{background:rgba(255,255,255,.07);border-radius:8px;height:4px;overflow:hidden;margin-bottom:20px}
    .hoja-progress-fill{height:100%;background:linear-gradient(90deg,var(--v3),var(--v4));border-radius:8px;transition:width .4s ease;width:0%}

    /* ── RESPONSIVE ── */
    @media(max-width:900px){.grid{grid-template-columns:1fr 1fr}.span3{grid-column:1/-1}.span2{grid-column:1/-1}.nav-links{display:none}}
    @media(max-width:600px){
      .navbar{padding:0 16px}.contenido{padding:20px}.hero{padding:28px 20px 0}
      .grid{grid-template-columns:1fr}.span2,.span3{grid-column:1/-1}
      .hero-inner{grid-template-columns:1fr;gap:16px;padding-bottom:40px}
      .hero-stats{gap:14px}.mfila{grid-template-columns:1fr}
    }
  </style>
</head>
<body>
...
<nav class="navbar">
  ...
  <div class="nav-links">
    ...
    <?php if ($tipo === 'empresa' || $tipo === 'negocio'): ?>
      <a href="#" class="nl" onclick="abrirPublicarVacante();return false;" style="color:var(--v4)">➕ Publicar vacante</a>
  ...
  <div class="nav-right">
    ...
    <div class="nav-notif" id="navNotif" title="Notificaciones">
      ...
      <div class="notif-panel" id="notifPanel">
        ...
        <div id="notifLista"><div class="notif-empty">Cargando…</div></div>
      </div>
    </div>
    <div class="nav-av" id="navAvatar" onclick="abrirModal()" title="Editar perfil">
      <?php if ($fotoUrl): ?><img src="<?= $fotoUrl ?>" alt="Foto" style="width:100%;height:100%;object-fit:cover"><?php else: ?><?= $inicial ?><?php endif; ?>
    </div>
    ...
    <div>
      <div class="hero-stats">
        ...
        <?php elseif ($subTipo === 'servicio'): ?>
          <div class="hs">
            <div class="hs-val"><?= $talento['calificacion'] ? number_format((float)$talento['calificacion'],1) : '—' ?></div>
...
<div class="contenido">
  ...
  <?php if (!$tieneVerificado): ?>
    <?php if ($estadoVerif === 'pendiente'): ?>
      <div class="alerta ap"><div class="a-ico">⏳</div><div class="a-txt"><strong>Documentos en revisión</strong><span>El administrador está revisando tu documento.</span></div></div>
    <?php elseif ($estadoVerif === 'rechazado'): ?>
      <div class="alerta ar"><div class="a-ico">❌</div><div class="a-txt"><strong>Verificación rechazada</strong><span><?= $notaRechazo ?: 'Intenta subir el documento con mejor calidad.' ?></span></div><a href="verificar_cuenta.php" class="a-btn">Reintentar</a></div>
    <?php else: ?>
      <div class="alerta as"><div class="a-ico">🪪</div><div class="a-txt"><strong>Verifica tu identidad</strong><span>Sube tu documento y obtén el badge verificado.</span></div><a href="verificar_cuenta.php" class="a-btn">Verificar ahora</a></div>
    <?php endif; ?>
  <?php else: ?>
    <div class="alerta av"><div class="a-ico">✅</div><div class="a-txt"><strong>Cuenta verificada</strong><span>Los empleadores ven tu badge de verificación.</span></div></div>
  <?php endif; ?>
  ...
  <div class="grid">
    ...
    <!-- ── MÉTRICAS SEGÚN TIPO ── -->
    <?php if ($tipo === 'empresa'): ?>
      <div class="card mini"><div class="m-ico ig">💼</div><div><div class="m-val"><?= $vacantesActivas ?></div><div class="m-lab">Vacantes activas</div><div class="m-sub" onclick="abrirPublicarVacante()">Publicar →</div></div></div>
      <div class="card mini"><div class="m-ico ia">👥</div><div><div class="m-val">0</div><div class="m-lab">Candidatos</div><div class="m-sub" onclick="location.href='talentos.php'">Ver talentos →</div></div></div>
      <div class="card mini"><div class="m-ico im">👁️</div><div><div class="m-val"><?= $vistasTotal ?></div><div class="m-lab">Vistas al perfil</div><div class="m-sub"><?= $vistas7dias > 0 ? '+'.$vistas7dias.' esta semana' : 'Ver en directorio →' ?></div></div></div>

    <?php elseif ($tipo === 'negocio' || ($extras['tipo_negocio_reg'] ?? '')): ?>
      <div class="card mini"><div class="m-ico ig">🏪</div><div><div class="m-val"><?= $vistasTotal ?></div><div class="m-lab">Vistas al negocio</div><div class="m-sub" onclick="location.href='negocios.php'"><?= $vistas7dias > 0 ? "+".$vistas7dias." esta semana" : "Ver directorio →" ?></div></div></div>
      <div class="card mini"><div class="m-ico io">💬</div><div><div class="m-val"><?= $chatNoLeidos ?></div><div class="m-lab">Mensajes</div><div class="m-sub" onclick="location.href='chat.php'">Ver chat →</div></div></div>
      <div class="card mini"><div class="m-ico im">⭐</div><div><div class="m-val"><?= $pct ?>%</div><div class="m-lab">Perfil completado</div><div class="m-sub" onclick="abrirModal()"><?= $pct < 100 ? 'Mejorar →' : '¡Perfecto! ✓' ?></div></div></div>

    <?php elseif ($subTipo === 'servicio' || !empty($talento['precio_desde'])): ?>
      <div class="card mini"><div class="m-ico ig">🎧</div><div><div class="m-val"><?= $talento['precio_desde'] ? '$'.number_format((float)$talento['precio_desde'],0,',','.') : '—' ?></div><div class="m-lab">Precio desde</div><div class="m-sub" onclick="location.href='servicios.php'">Ver servicios →</div></div></div>
      <div class="card mini"><div class="m-ico io">⭐</div><div><div class="m-val"><?= $talento['calificacion'] ? number_format((float)$talento['calificacion'],1) : '0' ?>/5</div><div class="m-lab">Calificación</div><div class="m-sub">Reseñas →</div></div></div>
      <div class="card mini"><div class="m-ico im">👁️</div><div><div class="m-val"><?= $vistasTotal ?></div><div class="m-lab">Vistas al perfil</div><div class="m-sub"><?= $vistas7dias > 0 ? '+'.$vistas7dias.' esta semana' : 'Hazte visible →' ?></div></div></div>

    <?php else: /* candidato */ ?>
      <div class="card mini"><div class="m-ico ig">📋</div><div><div class="m-val">0</div><div class="m-lab">Postulaciones</div><div class="m-sub">Empieza hoy →</div></div></div>
      <div class="card mini"><div class="m-ico im">👁️</div><div><div class="m-val" id="statVistas"><?= $vistasTotal ?></div><div class="m-lab">Vistas al perfil</div><div class="m-sub"><?= $vistas7dias > 0 ? "+".$vistas7dias." esta semana" : "Hazte visible →" ?></div></div></div>
      <div class="card mini"><div class="m-ico io">⭐</div><div><div class="m-val"><?= $pct ?>%</div><div class="m-lab">Perfil completado</div><div class="m-sub" onclick="abrirModal()"><?= $pct < 100 ? 'Mejorar →' : '¡Perfecto! ✓' ?></div></div></div>
    <?php endif; ?>
    ...
    <div class="card span3">
      ...
      <div class="ac-row">
        <?php if ($tipo === 'empresa'): ?>
          <a href="#" class="ac" onclick="abrirPublicarVacante();return false;" style="border-color:rgba(39,168,85,.3);background:rgba(39,168,85,.06)"><div class="ac-ico">➕</div><div class="ac-tit" style="color:var(--v4)">Publicar vacante</div><div class="ac-desc">Nueva oferta de empleo</div></a>
          <a href="talentos.php" class="ac"><div class="ac-ico">🌟</div><div class="ac-tit">Ver talentos</div><div class="ac-desc">Buscar candidatos</div></a>
          <a href="empresas.php" class="ac"><div class="ac-ico">🏢</div><div class="ac-tit">Mi empresa</div><div class="ac-desc">Ver en directorio</div></a>
        <?php elseif ($tipo === 'negocio'): ?>
          <a href="#" class="ac" onclick="abrirPublicarVacante();return false;" style="border-color:rgba(39,168,85,.3);background:rgba(39,168,85,.06)"><div class="ac-ico">➕</div><div class="ac-tit" style="color:var(--v4)">Publicar vacante</div><div class="ac-desc">Nueva oferta de empleo</div></a>
          <a href="negocios.php" class="ac"><div class="ac-ico">🏪</div><div class="ac-tit">Mi negocio</div><div class="ac-desc">Ver en directorio</div></a>
          <a href="Empleo.html" class="ac"><div class="ac-ico">💼</div><div class="ac-tit">Ver empleos</div><div class="ac-desc">Oportunidades</div></a>
        <?php elseif ($subTipo === 'servicio' || !empty($talento['precio_desde'])): ?>
          <a href="servicios.php" class="ac"><div class="ac-ico">🎧</div><div class="ac-tit">Mis servicios</div><div class="ac-desc">Ver en directorio</div></a>
          <a href="#" class="ac" onclick="abrirHoja();return false;" style="border-color:rgba(255,211,77,.2);background:rgba(255,211,77,.05)"><div class="ac-ico">📄</div><div class="ac-tit" style="color:var(--a4)">Mi Hoja de Vida</div><div class="ac-desc">Actualiza tu CV</div></a>
          <a href="Empleo.html" class="ac"><div class="ac-ico">💼</div><div class="ac-tit">Ver empleos</div><div class="ac-desc">Vacantes del Chocó</div></a>
        <?php else: /* candidato */ ?>
          <a href="Empleo.html" class="ac"><div class="ac-ico">🔍</div><div class="ac-tit">Buscar empleo</div><div class="ac-desc">Vacantes del Chocó</div></a>
          <a href="#" class="ac" onclick="abrirHoja();return false;" style="border-color:rgba(255,211,77,.2);background:rgba(255,211,77,.05)"><div class="ac-ico">📄</div><div class="ac-tit" style="color:var(--a4)">Mi Hoja de Vida</div><div class="ac-desc">Actualiza tu CV</div></a>
          <a href="talentos.php" class="ac"><div class="ac-ico">🌟</div><div class="ac-tit">Talentos</div><div class="ac-desc">Profesionales locales</div></a>
        <?php endif; ?>
        <a href="chat.php" class="ac">
          <div class="ac-ico">💬</div><div class="ac-tit">Mensajes</div>
          <?php if ($chatNoLeidos > 0): ?><span class="ac-badge"><?= $chatNoLeidos ?> sin leer</span><?php else: ?><div class="ac-desc">Sin nuevos</div><?php endif; ?>
        </a>
        <a href="verificar_cuenta.php" class="ac"><div class="ac-ico">🪪</div><div class="ac-tit">Verificación</div><div class="ac-desc"><?= $tieneVerificado ? '✅ Verificado' : 'Subir doc.' ?></div></a>
        <a href="convocatorias.php" class="ac"><div class="ac-ico">📢</div><div class="ac-tit">Convocatorias</div><div class="ac-desc">Sector público</div></a>
        <a href="empresas.php" class="ac"><div class="ac-ico">🏢</div><div class="ac-tit">Empresas</div><div class="ac-desc">Directorio</div></a>
        <a href="negocios.php" class="ac"><div class="ac-ico">🏪</div><div class="ac-tit">Negocios</div><div class="ac-desc">Locales & Emprendedores</div></a>
        <a href="servicios.php" class="ac"><div class="ac-ico">🎧</div><div class="ac-tit">Eventos</div><div class="ac-desc">DJs, fotógrafos...</div></a>
    ...
    <div class="card span2">
        ...
        <div class="ce-list">
          <?php foreach ($historialVacantes as $v): ?>
            <div class="hist-item">
              ...
              <div style="flex:1;min-width:0"><div class="hnom"><?= htmlspecialchars($v['titulo']) ?></div><div class="hmeta">📍 <?= htmlspecialchars($v['ciudad']??'Quibdó') ?> · <?= $v['activo'] ? '<span style="color:#4ade80;font-weight:700">Activa</span>' : '<span style="color:#94a3b8">Cerrada</span>' ?></div></div>
      ...
      <?php elseif ($tipo === 'empresa'): ?>
        <div style="text-align:center;padding:32px 20px;color:var(--ink3)">
          ...
          <a href="#" onclick="abrirPublicarVacante()" style="display:inline-block;margin-top:12px;padding:10px 22px;background:var(--v3);color:white;border-radius:10px;text-decoration:none;font-weight:800;font-size:13px">➕ Publicar primera vacante</a>
        </div>
      <?php elseif ($subTipo === 'servicio' && !empty($talento['generos'])): ?>
        <div class="ce-list">
          <?php foreach (array_slice(array_filter(array_map('trim', explode(',', $talento['generos']))), 0, 6) as $g): ?>
            <div class="ce-item"><div class="ce-ico">🎵</div><div class="ce-info"><div class="ce-nom"><?= htmlspecialchars($g) ?></div></div></div>
          <?php endforeach; ?>
        </div>
      <?php else: ?>
        <div class="ce-list">
          <?php foreach ([['💻','Ingeniero en Sistemas','Tech Chocó','Quibdó · $3M–$4.5M','Tiempo completo'],['🎨','Diseñador Gráfico','Agencia Creativa','Remoto · $1.5M–$2.5M','Freelance'],['🎵','Instructor de Música','Cultura Chocó','Quibdó · $800K–$1.2M','Medio tiempo'],['📊','Aux. Administrativo','Empresa Local','Quibdó · $1.2M–$1.6M','Tiempo completo']] as [$i,$n,$e,$m,$t]): ?>
            <div class="ce-item" onclick="location.href='Empleo.html'">
              ...
              <div class="ce-info"><div class="ce-nom"><?= $n ?></div><div class="ce-emp"><?= $e ?></div><div class="ce-met"><?= $m ?></div></div>
    ...
    <div class="card" style="display:flex;flex-direction:column">
      <div class="cp-head">
        <div class="cp-av" id="cpAvatar" onclick="abrirModal()">
          <?php if ($fotoUrl): ?><img src="<?= $fotoUrl ?>" alt="Foto" style="width:100%;height:100%;object-fit:cover;border-radius:18px"><?php else: ?><?= $inicial ?><?php endif; ?>
        </div>
        <div class="cp-nom" id="dNombre"><?= $tipo === 'empresa' ? $nombreEmpresa : ($tipo === 'negocio' ? $nombreNegocio : $nombreCompleto) ?></div>
      ...
      <div class="cp-body">
        ...
        <div class="cp-fil"><span class="cp-ico">📞</span><span id="dTelefono"><?= $telefono ?: 'Teléfono no registrado' ?></span></div>
        <div class="cp-fil"><span class="cp-ico">✉️</span><span><?= $correo ?></span></div>
        <?php if ($tipo === 'empresa' && !empty($extras['nit']??$ep['nit']??'')): ?>
          <div class="cp-fil"><span class="cp-ico">🏛️</span><span>NIT: <?= htmlspecialchars($extras['nit']??$ep['nit']??'') ?></span></div>
        <?php endif; ?>
        <?php if ($tipo === 'negocio' && !empty($extras['whatsapp_neg']??$np['whatsapp']??'')): ?>
          <div class="cp-fil"><span class="cp-ico">💬</span><span>WhatsApp: <?= htmlspecialchars($extras['whatsapp_neg']??$np['whatsapp']??'') ?></span></div>
        <?php endif; ?>
        <?php if ($subTipo === 'servicio' && !empty($talento['precio_desde'])): ?>
          <div class="cp-fil"><span class="cp-ico">💰</span><span>Desde $<?= number_format((float)$talento['precio_desde'],0,',','.') ?></span></div>
        <?php endif; ?>
      ...
      <?php if ($tipo === 'candidato' || $subTipo === 'servicio'): ?>
        <div style="padding:0 22px">
          <div class="vis-row">
            <div><div class="vis-lab">Visible en <?= $subTipo === 'servicio' ? 'Servicios' : 'Talentos' ?></div>
              <div class="vis-sub">Aparece en el directorio público</div></div>
            <div style="display:flex;align-items:center;gap:8px">
              <span id="pvBadge" class="pv-chip <?= ($talento['visible']??0) ? 'ok' : 'off' ?>"><?= ($talento['visible']??0) ? '🟢 Visible' : '🟡 Oculto' ?></span>
              <label class="tog"><input type="checkbox" <?= ($talento['visible']??0) ? 'checked' : '' ?> onchange="toggleVis(this.checked)"><span class="tog-sl"></span></label>
            </div>
          </div>
        </div>
      <?php elseif ($tipo === 'empresa'): ?>
        <div style="padding:0 22px">
          <div class="vis-row">
            <div><div class="vis-lab">Visible en Empresas</div><div class="vis-sub">Aparece en el directorio público</div></div>
            <span class="pv-chip <?= ($ep['visible_admin']??1) ? 'ok' : 'off' ?>"><?= ($ep['visible_admin']??1) ? '🟢 Visible' : '🟡 Oculto' ?></span>
          </div>
        </div>
      <?php elseif ($tipo === 'negocio'): ?>
        <div style="padding:0 22px">
          <div class="vis-row">
            <div><div class="vis-lab">Visible en Negocios</div><div class="vis-sub">Aparece en el directorio público</div></div>
            <span class="pv-chip <?= ($np['visible_admin']??1) ? 'ok' : 'off' ?>"><?= ($np['visible_admin']??1) ? '🟢 Visible' : '🟡 Oculto' ?></span>
          </div>
        </div>
      <?php endif; ?>
      ...
      <a href="<?= $tipo==='empresa' ? 'empresas.php#u'.$usuario['id'] : ($tipo==='negocio' ? 'negocios.php#u'.$usuario['id'] : ($subTipo==='servicio' ? 'servicios.php' : 'talentos.php')) ?>" class="btn-sec">🌐 Ver mi perfil en directorio</a>
    </div>
    ...
    <div class="card span2">
      ...
      <div class="act-list">
        <div class="act-it"><div class="act-pt pv">🎉</div><div class="act-tx"><div class="act-n">¡Cuenta creada!</div><div class="act-f"><?= $fechaRegistro ?></div></div></div>
        <div class="act-it"><div class="act-pt pa">👀</div><div class="act-tx"><div class="act-n">Exploraste empleos</div><div class="act-f">Hoy</div></div></div>
        <?php if ($tieneVerificado): ?>
          <div class="act-it"><div class="act-pt pv">✅</div><div class="act-tx"><div class="act-n">Cuenta verificada</div><div class="act-f">Badge asignado</div></div></div>
        <?php endif; ?>
        <?php if ($tipo === 'empresa' && $vacantesActivas > 0): ?>
          <div class="act-it"><div class="act-pt ig">💼</div><div class="act-tx"><div class="act-n"><?= $vacantesActivas ?> vacante<?= $vacantesActivas>1?'s':'' ?> activa<?= $vacantesActivas>1?'s':'' ?></div><div class="act-f">Empresa activa</div></div></div>
        <?php endif; ?>
        <?php if (!empty($talento['precio_desde'])): ?>
          <div class="act-it"><div class="act-pt po">🎧</div><div class="act-tx"><div class="act-n">Servicio configurado</div><div class="act-f">Apareces en Eventos</div></div></div>
        <?php endif; ?>
      </div>
    </div>
...
<?php if ($tipo === 'candidato' || $subTipo === 'servicio'): ?>
  ...
  ?>
  <!-- ══ GALERÍA DE EVIDENCIAS ══════════════════════════════════ -->
  <div style="margin-top:28px;background:var(--blan,#fff);border:1px solid var(--bord,#e5e7eb);border-radius:18px;padding:28px">
    <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px">
      <div>
        <div style="font-size:13px;font-weight:800;color:#6b7280;text-transform:uppercase;letter-spacing:.7px;margin-bottom:4px">📸 Mi galería de evidencias</div>
        <div style="font-size:13px;color:#374151">
          Fotos y videos de tus servicios — para que los clientes vean tu trabajo.
          <?php if (!$tieneSelvaVerde): ?>
            <strong style="color:<?= $galeriaTotal >= 15 ? '#dc2626' : '#374151' ?>"><?= $galeriaTotal ?>/15 usados</strong>
          <?php else: ?>
            <span style="background:#dcfce7;color:#166534;padding:2px 10px;border-radius:20px;font-weight:700;font-size:12px">🌿 Selva Verde — Ilimitado</span>
          <?php endif; ?>
        </div>
      </div>
      <?php if ($puedeSubir): ?>
      ...
      <?php else: ?>
        <div style="padding:10px 16px;background:#fef3c7;border:1px solid #fde68a;border-radius:10px;font-size:13px;color:#92400e">
          🌿 <strong>Límite alcanzado.</strong>
          <a href="mailto:soporte@quibdoconecta.co" style="color:#1f9d55;font-weight:700">Activa Selva Verde</a> para ilimitado.
        </div>
      <?php endif; ?>
    </div>
    ...
    <?php if (empty($galeriaItems)): ?>
      <div style="text-align:center;padding:40px 20px;border:2px dashed #e5e7eb;border-radius:12px;color:#9ca3af">
        ...
        <div style="font-size:13px">Sube fotos de tus eventos o videos de tus presentaciones para atraer más clientes.</div>
      </div>
    <?php else: ?>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px" id="galeriaGrid">
        ...
        ?>
          <div style="position:relative;border-radius:10px;overflow:hidden;background:#f9fafb;border:1px solid #e5e7eb;aspect-ratio:1" id="gitem-<?= $gi['id'] ?>">
            <?php if ($isVideo && $gi['url_video']): ?>
              <a href="<?= htmlspecialchars($gi['url_video']) ?>" target="_blank" style="display:block;height:100%;position:relative">
                <?php if ($thumb): ?><img src="<?= $thumb ?>" style="width:100%;height:100%;object-fit:cover" loading="lazy"><?php endif; ?>
            ...
            <?php elseif ($gi['archivo']): ?>
              <?php if ($isVideo): ?>
                <video src="uploads/galeria/<?= htmlspecialchars($gi['archivo']) ?>" style="width:100%;height:100%;object-fit:cover" controls preload="none"></video>
              <?php else: ?>
                <img src="uploads/galeria/<?= htmlspecialchars($gi['archivo']) ?>" style="width:100%;height:100%;object-fit:cover;cursor:pointer" loading="lazy"
                     onclick="verImagenGaleria('uploads/galeria/<?= htmlspecialchars($gi['archivo']) ?>','<?= htmlspecialchars($gi['titulo']??'') ?>')">
              <?php endif; ?>
            <?php endif; ?>
            <?php if ($gi['titulo']): ?>
              <div style="position:absolute;bottom:0;left:0;right:0;padding:6px 8px;background:rgba(0,0,0,.55);color:#fff;font-size:11px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
  ...
  <div class="psec" id="psec-educacion">
    ...
    <div class="psec-list" id="edu-list">
      <div style="text-align:center;padding:30px 0;color:var(--ink3);font-size:13px">
        ...
        <br><button onclick="abrirFormEdu()" style="margin-top:12px;padding:8px 20px;border:1.5px dashed rgba(163,240,181,.3);border-radius:20px;background:none;color:var(--vlima);font-size:13px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s">+ Agregar educación</button>
      </div>
    </div>
  </div>
  ...
  <div class="psec" id="psec-cert">
    ...
    <div class="psec-list" id="cert-list">
      <div style="text-align:center;padding:30px 0;color:var(--ink3);font-size:13px">
        ...
        <br><button onclick="abrirFormCert()" style="margin-top:12px;padding:8px 20px;border:1.5px dashed rgba(163,240,181,.3);border-radius:20px;background:none;color:var(--vlima);font-size:13px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s">+ Agregar certificación</button>
      </div>
    </div>
  </div>
  ...
  <div class="psec" id="psec-apt">
    ...
    <div class="psec-list" id="apt-list">
      ...
      <div style="text-align:center;padding:30px 0;color:var(--ink3);font-size:13px">
        ...
        <br><button onclick="abrirFormApt()" style="margin-top:12px;padding:8px 20px;border:1.5px dashed rgba(163,240,181,.3);border-radius:20px;background:none;color:var(--vlima);font-size:13px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s">+ Agregar aptitudes</button>
      </div>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div class="hoja-modal-box" style="max-width:640px">
    ...
    <div class="modal-pad">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px">
        <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--v2),var(--v3));display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">➕</div>
        <div>
          <div class="mtit" style="margin:0;font-size:20px">Publicar <em style="color:var(--vlima);font-style:normal">Vacante</em></div>
          <p class="msub" style="margin:0;font-size:12px">Conecta con el mejor talento del Chocó en minutos</p>
        </div>
      </div>
      <div class="hoja-progress-track" style="margin-top:14px"><div class="hoja-progress-fill" id="vacProgressFill"></div></div>
      <div class="mmsg" id="vacMsg"></div>
      <div class="hoja-sec"><div class="hoja-sec-num">1</div> Información de la vacante</div>
      ...
      <div class="hoja-fila">
        <div class="hoja-gr"><label>Empresa <span style="color:#ff6b6b">*</span></label>
          <input type="text" id="vacEmpresa" placeholder="Nombre de tu empresa" oninput="vacProgress()" value="<?= htmlspecialchars($tipo==='empresa' ? ($ep['nombre_empresa']??$usuario['nombre']??'') : ($np['nombre_negocio']??$usuario['nombre']??'')) ?>">
        </div>
        <div class="hoja-gr"><label>Ubicación <span style="color:#ff6b6b">*</span></label>
          <input type="text" id="vacUbicacion" placeholder="Ej: Quibdó, Chocó" oninput="vacProgress()" value="<?= htmlspecialchars($usuario['ciudad']??'Quibdó, Chocó') ?>">
        </div>
      </div>
      <div class="hoja-divider"></div>
      <div class="hoja-sec"><div class="hoja-sec-num">2</div> Condiciones laborales</div>
      <div class="hoja-fila">
        <div class="hoja-gr"><label>Tipo de empleo <span style="color:#ff6b6b">*</span></label>
          <select id="vacTipo" onchange="vacProgress()">
            ...
            <option>Remoto</option><option>Freelance</option><option>Prácticas</option>
          </select>
        </div>
        <div class="hoja-gr"><label>Salario</label><input type="text" id="vacSalario" placeholder="Ej: $2.000.000 / mes"></div>
      </div>
      <div class="hoja-fila">
        <div class="hoja-gr"><label>Categoría</label>
          <select id="vacCategoria">
            <option value="">Seleccione</option>
            <option>Administrativo</option><option>Tecnología</option><option>Educación</option>
            <option>Salud</option><option>Gastronomía</option><option>Arte &amp; Música</option>
            <option>Transporte</option><option>Técnico</option><option>Otro</option>
          </select>
        </div>
        <div class="hoja-gr"><label>Fecha límite</label><input type="date" id="vacFecha"></div>
      </div>
      <div class="hoja-divider"></div>
      <div class="hoja-sec"><div class="hoja-sec-num">3</div> Descripción y requisitos</div>
      <div class="hoja-fila" style="grid-template-columns:1fr">
        <div class="hoja-gr full"><label>Descripción del cargo <span style="color:#ff6b6b">*</span></label>
          <textarea id="vacDesc" rows="3" placeholder="Funciones y responsabilidades del cargo…" oninput="vacProgress();vacCount(this,'vacDescC',500)"></textarea>
          <div style="text-align:right;font-size:11px;color:rgba(255,255,255,.25);margin-top:2px"><span id="vacDescC">0</span>/500</div>
        </div>
      </div>
      <div class="hoja-fila" style="grid-template-columns:1fr">
        <div class="hoja-gr full"><label>Requisitos <span style="color:#ff6b6b">*</span></label>
          <textarea id="vacReq" rows="3" placeholder="Experiencia, estudios, habilidades requeridas…" oninput="vacProgress();vacCount(this,'vacReqC',500)"></textarea>
          <div style="text-align:right;font-size:11px;color:rgba(255,255,255,.25);margin-top:2px"><span id="vacReqC">0</span>/500</div>
        </div>
      </div>
      <div style="display:flex;gap:10px;margin-top:20px">
        <button onclick="cerrarPublicarVacante()" style="flex:1;padding:13px;background:rgba(255,255,255,.07);border:1.5px solid rgba(255,255,255,.1);color:rgba(255,255,255,.7);border-radius:14px;font-size:14px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer" onmouseover="this.style.background='rgba(255,255,255,.12)'" onmouseout="this.style.background='rgba(255,255,255,.07)'">Cancelar</button>
        <button id="btnGuardarVac" onclick="publicarVacante()" style="flex:2;padding:13px;background:linear-gradient(135deg,var(--v1),var(--v2) 50%,var(--v3));color:#fff;border:none;border-radius:14px;font-size:14px;font-weight:800;font-family:'DM Sans',sans-serif;cursor:pointer;box-shadow:0 6px 20px rgba(39,168,85,.4)">🚀 Publicar vacante</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal-ov" id="modalHoja">
  <div class="hoja-modal-box">
    ...
    <div class="modal-pad">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px">
        <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--a1),var(--a3));display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">📄</div>
        <div>
          <div class="mtit" style="margin:0;font-size:20px">Mi Hoja de <em style="color:var(--vlima);font-style:normal">Vida</em></div>
          <p class="msub" style="margin:0;font-size:12px">Completa tu perfil y sé encontrado por empresas del Chocó</p>
        </div>
      </div>
      <div class="hoja-progress-track" style="margin-top:14px"><div class="hoja-progress-fill" id="hojaProgressFill"></div></div>
      <div class="mmsg" id="hojaMsg"></div>
      <div class="hoja-sec"><div class="hoja-sec-num">1</div> Datos personales</div>
      <div class="hoja-fila">
        <div class="hoja-gr"><label>Nombre completo <span style="color:#ff6b6b">*</span></label><input type="text" id="hNombre" oninput="hojaProgress()" value="<?= htmlspecialchars(trim(($usuario['nombre']??'').' '.($usuario['apellido']??''))) ?>"></div>
        <div class="hoja-gr"><label>Correo <span style="color:#ff6b6b">*</span></label><input type="email" id="hCorreo" oninput="hojaProgress()" value="<?= htmlspecialchars($usuario['correo']??'') ?>"></div>
      </div>
      <div class="hoja-fila">
        <div class="hoja-gr"><label>Teléfono <span style="color:#ff6b6b">*</span></label><input type="tel" id="hTelefono" placeholder="300 123 4567" oninput="hojaProgress()" value="<?= htmlspecialchars($usuario['telefono']??'') ?>"></div>
        <div class="hoja-gr"><label>Ciudad</label><input type="text" id="hCiudad" placeholder="Quibdó, Chocó" value="<?= htmlspecialchars($usuario['ciudad']??'') ?>"></div>
      </div>
      <div class="hoja-fila">
        <div class="hoja-gr"><label>Profesión / Área</label><input type="text" id="hProfesion" placeholder="Ej: Contador, DJ, Diseñador…" value="<?= htmlspecialchars($talento['profesion']??'') ?>"></div>
        <div class="hoja-gr"><label>Disponibilidad</label>
          <select id="hDisponibilidad">
            <option value="">Seleccione</option>
            <option>Inmediata</option><option>En 15 días</option><option>En 1 mes</option><option>Solo fines de semana</option>
          </select>
        </div>
      </div>
      <div class="hoja-fila">
        <div class="hoja-gr full"><label>Perfil profesional</label>
          <textarea id="hPerfil" rows="3" placeholder="Breve descripción de quién eres y qué buscas…" oninput="countHojaChars(this,'hPerfilCount',300)"><?= htmlspecialchars($talento['bio']??'') ?></textarea>
          <div style="text-align:right;font-size:11px;color:rgba(255,255,255,.25)"><span id="hPerfilCount"><?= mb_strlen($talento['bio']??'') ?></span>/300</div>
        </div>
      </div>
      <div class="hoja-divider"></div>
      <div class="hoja-sec"><div class="hoja-sec-num">2</div> Experiencia laboral</div>
      <div id="listaExp"></div>
      <button class="hoja-btn-add" onclick="agregarExp()">+ Agregar experiencia laboral</button>
      <div class="hoja-divider"></div>
      <div class="hoja-sec"><div class="hoja-sec-num">3</div> Formación académica</div>
      <div id="listaForm"></div>
      <button class="hoja-btn-add" onclick="agregarForm()">+ Agregar formación académica</button>
      <div class="hoja-divider"></div>
      <div class="hoja-sec"><div class="hoja-sec-num">4</div> Habilidades e idiomas</div>
      <div class="hoja-fila">
        <div class="hoja-gr"><label>Habilidades técnicas</label><input type="text" id="hHabTec" placeholder="Excel, Python, Photoshop…" value="<?= htmlspecialchars($talento['skills']??'') ?>"></div>
        <div class="hoja-gr"><label>Habilidades blandas</label><input type="text" id="hHabBland" placeholder="Liderazgo, Trabajo en equipo…"></div>
      </div>
      <div class="hoja-fila">
        <div class="hoja-gr"><label>Idiomas</label><input type="text" id="hIdiomas" placeholder="Español (nativo), Inglés (básico)…"></div>
        <div class="hoja-gr"><label>Portafolio / LinkedIn</label><input type="url" id="hPortafolio" placeholder="https://miportafolio.com"></div>
      </div>
      <div class="hoja-divider"></div>
      <div class="hoja-sec"><div class="hoja-sec-num">5</div> Certificados y cursos</div>
      <div id="listaCert"></div>
      <button class="hoja-btn-add" onclick="agregarCert()">+ Agregar certificado o curso</button>
      <div style="display:flex;gap:10px;margin-top:24px">
        <button onclick="cerrarHoja()" style="flex:1;padding:13px;background:rgba(255,255,255,.07);border:1.5px solid rgba(255,255,255,.1);color:rgba(255,255,255,.7);border-radius:14px;font-size:14px;font-weight:700;font-family:'DM Sans',sans-serif;cursor:pointer" onmouseover="this.style.background='rgba(255,255,255,.12)'" onmouseout="this.style.background='rgba(255,255,255,.07)'">Cancelar</button>
        <button id="btnGuardarHoja" onclick="guardarHoja()" style="flex:2;padding:13px;background:linear-gradient(135deg,var(--v1),var(--v2) 50%,var(--v3));color:#fff;border:none;border-radius:14px;font-size:14px;font-weight:800;font-family:'DM Sans',sans-serif;cursor:pointer;box-shadow:0 6px 20px rgba(39,168,85,.4)">💾 Guardar hoja de vida</button>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="modal-box">
    ...
    <div class="modal-pad">
      ...
      <div style="display:flex;align-items:center;gap:16px;margin-bottom:18px">
        <div id="fotoPreview" style="width:72px;height:72px;border-radius:18px;background:linear-gradient(135deg,var(--v1),var(--v3));display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:900;color:white;overflow:hidden;flex-shrink:0;cursor:pointer;border:2px solid rgba(163,240,181,.2)" onclick="document.getElementById('fotoInput').click()">
          <?php if ($fotoUrl): ?><img src="<?= $fotoUrl ?>" id="fotoImgPreview" style="width:100%;height:100%;object-fit:cover"><?php else: ?><span id="fotoInicialPreview"><?= $inicial ?></span><?php endif; ?>
        </div>
        <div>
          <input type="file" id="fotoInput" accept="image/jpeg,image/png,image/webp" style="display:none" onchange="abrirCrop(this)">
          <button onclick="document.getElementById('fotoInput').click()" style="padding:8px 14px;border-radius:10px;background:var(--v3);color:white;border:none;font-size:13px;font-weight:700;cursor:pointer">📷 Cambiar foto</button>
      ...
      <div class="crop-modal" id="cropModal" style="display:none">
        <div class="crop-inner">
          <div style="font-size:16px;font-weight:800;color:var(--vlima);margin-bottom:14px;text-align:center">✂️ Encuadra tu foto</div>
          <div style="position:relative;width:100%;height:280px;overflow:hidden;border-radius:12px;background:#000;display:flex;align-items:center;justify-content:center">
            <img id="cropImg" style="max-width:100%;display:block">
          </div>
          <p style="font-size:12px;color:var(--ink3);text-align:center;margin:10px 0">Arrastra y usa el zoom para encuadrar</p>
          <div style="display:flex;gap:10px;margin-top:6px">
            <button onclick="cancelarCrop()" style="flex:1;padding:11px;border-radius:10px;border:1px solid var(--borde);background:rgba(255,255,255,.05);font-size:13px;font-weight:700;cursor:pointer;color:var(--ink2);font-family:'DM Sans',sans-serif">Cancelar</button>
            <button onclick="confirmarCrop()" id="btnConfirmarCrop" style="flex:2;padding:11px;border-radius:10px;border:none;background:linear-gradient(135deg,var(--v1),var(--v3));color:white;font-size:13px;font-weight:800;cursor:pointer;font-family:'DM Sans',sans-serif">✅ Usar esta foto</button>
          </div>
        </div>
      </div>
      ...
      <div class="mfila">
        <div class="mgr"><label>Nombre *</label><input type="text" id="editNombre" value="<?= htmlspecialchars($usuario['nombre']) ?>"></div>
        <div class="mgr"><label>Apellido</label><input type="text" id="editApellido" value="<?= htmlspecialchars($usuario['apellido']??'') ?>"></div>
      </div>
      <div class="mfila">
        <div class="mgr"><label>Teléfono</label><input type="tel" id="editTelefono" value="<?= htmlspecialchars($usuario['telefono']??'') ?>" placeholder="300 123 4567"></div>
        <div class="mgr"><label>Ciudad</label><input type="text" id="editCiudad" value="<?= htmlspecialchars($usuario['ciudad']??'') ?>" placeholder="Quibdó"></div>
      </div>

      <!-- Info extra según tipo -->
      <?php if ($tipo === 'empresa'): ?>
        <div style="background:rgba(26,86,219,.08);border:1px solid rgba(26,86,219,.2);border-radius:12px;padding:12px 16px;font-size:12px;color:var(--rcielo);margin:8px 0">
          🏢 Los datos de empresa (razón social, NIT, sector, cámara de comercio) son actualizados por el administrador desde el panel de gestión.
        </div>
        <?php if (!empty($extras['razon_social']??$ep['nombre_empresa']??'')): ?>
          <div class="mfila"><div class="mgr full"><label>Empresa</label><input type="text" value="<?= htmlspecialchars($extras['razon_social']??$ep['nombre_empresa']??$usuario['nombre']??'') ?>" readonly style="opacity:.5"></div></div>
          <div class="mfila">
            <div class="mgr"><label>Sector</label><input type="text" value="<?= htmlspecialchars($extras['sector']??$ep['sector']??'') ?>" readonly style="opacity:.5"></div>
            <div class="mgr"><label>NIT</label><input type="text" value="<?= htmlspecialchars($extras['nit']??$ep['nit']??'') ?>" readonly style="opacity:.5"></div>
          </div>
        <?php endif; ?>
      <?php elseif ($tipo === 'negocio'): ?>
        <div style="background:rgba(245,200,0,.07);border:1px solid rgba(245,200,0,.2);border-radius:12px;padding:12px 16px;font-size:12px;color:var(--acrem);margin:8px 0">
          🏪 Los datos del negocio (nombre, categoría, WhatsApp) son actualizados desde el panel de gestión o por el administrador.
        </div>
      <?php elseif ($subTipo === 'servicio'): ?>
        <div style="background:rgba(245,200,0,.07);border:1px solid rgba(245,200,0,.2);border-radius:12px;padding:12px 16px;font-size:12px;color:var(--acrem);margin:8px 0">
          🎧 Tu precio, géneros y tipo de servicio son activados por el administrador. Para actualizar tu perfil de servicios contacta al equipo de QuibdóConecta.
        </div>
      <?php else: ?>
        ...
        <div class="mfila">
          <div class="mgr full"><label>Profesión / Área</label>
            <input type="text" id="editProfesion" value="<?= htmlspecialchars($talento['profesion'] ?? $profesionTipo ?? '') ?>" placeholder="Ej: Desarrollador Web, Contador, DJ…">
          </div>
        </div>
        <div class="mfila">
          <div class="mgr full"><label>Biografía / Descripción</label>
            <textarea id="editBio" rows="3" placeholder="Cuéntale a las empresas quién eres y qué ofreces…" style="resize:vertical;font-family:'DM Sans',sans-serif;font-size:14px;padding:10px 14px;border-radius:12px;background:rgba(255,255,255,.05);border:1px solid var(--borde);color:var(--ink1);width:100%;box-sizing:border-box"><?= htmlspecialchars($talento['bio'] ?? '') ?></textarea>
          </div>
        </div>
        <div class="mfila">
          <div class="mgr full"><label>Habilidades <span style="font-weight:400;opacity:.6">(separadas por coma)</span></label>
            <input type="text" id="editSkills" value="<?= htmlspecialchars($talento['skills'] ?? '') ?>" placeholder="Ej: PHP, JavaScript, Diseño, Liderazgo…">
          </div>
        </div>
      <?php endif; ?>
...
<script>
  ...
  async function publicarVacante(){
    ...
    try{
      ...
      if(j.ok){m.textContent='✅ ¡Vacante publicada!';m.className='mmsg success';m.style.display='block';
...
<div class="modal-ov" id="modal-evidencia">
  <div class="modal-box" style="max-width:520px">
    ...
    <div class="modal-pad">
      ...
      <p style="font-size:13px;color:#6b7280;margin-bottom:20px">Fotos de tus eventos, videos de presentaciones — lo que tus clientes necesitan ver para contratarte.</p>
      <form id="form-evidencia" enctype="multipart/form-data">
        ...
        <div style="margin-top:12px">
          ...
          <input name="titulo" type="text" class="minput" placeholder="ej: Boda en Quibdó · Agosto 2025" maxlength="100">
        </div>
        <div style="margin-top:10px">
          <label class="mlabel">Descripción (opcional)</label>
          <textarea name="descripcion" class="minput" rows="2" style="resize:vertical" placeholder="Cuéntale al cliente qué muestra esta foto/video"></textarea>
        </div>

        <p id="ev-msg" style="display:none;font-size:13px;margin-top:12px;font-weight:600"></p>

        <button type="button" id="btn-ev-subir" onclick="subirEvidencia()" class="btn-save" style="width:100%;margin-top:16px">
          ⬆️ Subir
        </button>
      </form>
    </div>
...
<div class="modal-ov" id="modal-edu">
  <div class="modal-box" style="max-width:520px">
    ...
    <div class="modal-pad">
      ...
      <div class="mfila">
        <div class="mgr full"><label>Institución *</label><input type="text" id="edu-inst" placeholder="Ej: Universidad Tecnológica del Chocó"></div>
      </div>
      <div class="mfila">
        <div class="mgr full"><label>Título / Carrera *</label><input type="text" id="edu-titulo" placeholder="Ej: Ingeniería de Sistemas, Bachillerato…"></div>
      </div>
      <div class="mfila">
        <div class="mgr"><label>Año inicio</label><input type="number" id="edu-inicio" min="1990" max="2030" placeholder="2020"></div>
        <div class="mgr"><label>Año fin</label><input type="number" id="edu-fin" min="1990" max="2030" placeholder="2024 (vacío = Actualidad)"></div>
      </div>
      <div class="mfila">
...
<div class="modal-ov" id="modal-cert">
  <div class="modal-box" style="max-width:520px">
    ...
    <div class="modal-pad">
      ...
      <div class="mfila">
        <div class="mgr full"><label>Nombre del certificado *</label><input type="text" id="cert-nom" placeholder="Ej: Técnico en Programación de Software"></div>
      </div>
      <div class="mfila">
        <div class="mgr full"><label>Institución / Organización *</label><input type="text" id="cert-org" placeholder="Ej: SENA, Coursera, Google…"></div>
      </div>
      <div class="mfila">
        <div class="mgr"><label>Fecha expedición</label><input type="month" id="cert-fecha"></div>
        <div class="mgr"><label>ID / Código credencial</label><input type="text" id="cert-id" placeholder="Opcional"></div>
      </div>
      <div class="mfila">
        <div class="mgr full"><label>URL de credencial</label><input type="url" id="cert-url" placeholder="https://... (opcional)"></div>
      </div>
      <div class="mfila">
    ...
    <div class="modal-pad">
      ...
      <div class="mfila">
        <div class="mgr full"><label>Habilidades técnicas (separadas por coma)</label>
          <input type="text" id="apt-tec" placeholder="PHP, JavaScript, Excel, Photoshop…" value="<?= htmlspecialchars($talento['skills']??'') ?>">
        </div>
      </div>
      <div class="mfila">
