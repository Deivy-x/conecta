...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'editar_servicio') {
    ...
    try {
      $db->exec("ALTER TABLE talento_perfil ADD COLUMN IF NOT EXISTS instagram VARCHAR(100) DEFAULT ''");
    } catch (Exception $e) {
    }
    try {
      $db->prepare("UPDATE talento_perfil SET instagram=? WHERE usuario_id=? ORDER BY id DESC LIMIT 1")->execute([$instagram, $usuario['id']]);
    } catch (Exception $e) {
    }
  ...
  if ($action === 'toggle_vis') {
    ...
    if ($visible === 1 && function_exists('tieneBeneficio')) {
      if (!tieneBeneficio($db, $usuario['id'], 'portafolio')) {
        echo json_encode([
          'ok' => false,
          'msg' => '🌿 Necesitas el plan Verde Selva o superior para aparecer en el directorio de servicios.',
          'requiere_plan' => true
        ]);
        exit;
      }
    ...
    $tpRow = $tpChk->fetch();
    if ($tpRow)
      $db->prepare("UPDATE talento_perfil SET visible=? WHERE id=?")->execute([$visible, $tpRow['id']]);
  ...
  if ($action === 'subir_evidencia') {
    ...
    $cnt = $db->prepare("SELECT COUNT(*) FROM talento_galeria WHERE usuario_id=? AND activo=1");
    $cnt->execute([$usuario['id']]);
    $total = (int) $cnt->fetchColumn();
    $limite = $tienePortafolio ? PHP_INT_MAX : 15;
    if ($total >= $limite) {
      echo json_encode(['ok' => false, 'msg' => 'Alcanzaste el límite de 15 archivos. Activa el plan Selva Verde 🌿 para galería ilimitada.']);
      exit;
    }
  ...
  <style>
    ...
    *::after {
    ...
    .sidebar {
      ...
      border-right: 2px solid rgba(109, 40, 217, .10);
      display: flex;
    ...
    .sidebar-plan {
      background: linear-gradient(135deg, #6d28d9 0%, #8b5cf6 60%, #1a3a6b 100%);
      border-radius: var(--radius);
      padding: 14px;
      color: #fff;
      margin-bottom: 10px
    ...
    .dash-avatar-btn.open {
      ...
      box-shadow: 0 4px 20px rgba(109, 40, 217, .22);
    ...
    /* ── HERO STRIP ── */
    .hero-strip {
      background: linear-gradient(135deg, #fff 0%, #f5f3ff 60%, #fffbeb 100%);
    ...
    .hero-strip::before {
      ...
      right: 0;
      height: 4px;
      background: linear-gradient(90deg, #6d28d9 33.3%, #f59e0b 33.3% 66.6%, #1a3a6b 66.6%)
    }
    ...
    .card {
      ...
      border-radius: var(--radius);
      overflow: hidden;
      box-shadow: 0 1px 4px rgba(109, 40, 217, .06)
    }
...
<body>
      ...
      <div style="position:relative" id="dashUserWidget">
        ...
        <div class="dash-dropdown" id="dashDropdown">
          <div class="dash-drop-header">
            <div class="dash-drop-av"><?php if ($fotoUrl): ?><img src="<?= $fotoUrl ?>"
                  alt="Foto"><?php else: ?><?= $inicial ?><?php endif; ?></div>
            <div>
  ...
  <main class="main">
    ...
    <?php if (!$tieneVerificado): ?>
      ...
      <?php elseif ($estadoVerif === 'rechazado'): ?>
        <div class="alert-bar ar">
          <div class="a-ico">❌</div>
          <div class="a-txt"><strong>Verificación rechazada</strong><span>Intenta subir el documento con mejor
              calidad.</span></div><a href="verificar_cuenta.php" class="a-btn">Reintentar</a>
        </div>
      <?php else: ?>
    ...
    <?php else: ?>
      <div class="alert-bar av">
        <div class="a-ico">✅</div>
        <div class="a-txt"><strong>Cuenta verificada</strong><span>Los clientes ven tu badge de verificación.</span></div>
      </div>
    <?php endif; ?>
    ...
    <div class="hero-strip">
      ...
      <div class="hero-stats">
        ...
        <div class="hs">
          <div class="hs-val"><?= $calificacion ?>/5</div>
          <div class="hs-lab">Calific.</div>
        </div>
        <div class="hs">
          <div class="hs-val"><?= $vistasTotal ?></div>
          <div class="hs-lab">Vistas</div>
        </div>
      </div>
    ...
    <div class="dashboard-grid">
      ...
      <div class="col-4">
        <div class="card" style="height:100%">
          ...
          <div class="profile-card">
            ...
            <div class="pc-rows">
              <?php if ($precioDes): ?>
                <div class="pc-row"><span class="pc-row-ico">💰</span><span>Desde <?= $precioDes ?></span></div>
              <?php endif; ?>
              <?php if ($ciudad): ?>
                <div class="pc-row"><span class="pc-row-ico">📍</span><span><?= $ciudad ?></span></div><?php endif; ?>
              <?php if ($telefono): ?>
                <div class="pc-row"><span class="pc-row-ico">📞</span><span><?= $telefono ?></span></div><?php endif; ?>
            ...
            <div class="vis-row">
              ...
              <div style="display:flex;align-items:center;gap:7px">
                <span id="visChip"
                  class="pv-chip <?= $visible ? 'ok' : 'off' ?>"><?= $visible ? '🟣 Visible' : '🟡 Oculto' ?></span>
                <label class="tog"><input type="checkbox" <?= $visible ? 'checked' : '' ?>
      ...
      <div class="col-8">
        <div class="card">
          <div class="card-header">
            <div class="card-title">📸 Portafolio <span
                style="background:var(--brand-light);color:var(--brand);font-size:10px;padding:2px 8px;border-radius:20px;font-weight:700;margin-left:6px"><?= $galeriaTotal ?>
                fotos</span></div>
            <button class="btn-primary" onclick="abrirModalPortafolio()" style="padding:6px 14px;font-size:12px">+
              Añadir</button>
          </div>
          ...
          <?php else: ?>
            <div class="gallery-empty" style="margin:16px">
              ...
              <div style="font-size:13px;font-weight:700;color:var(--ink2);margin-bottom:4px">Sin fotos de portafolio
              </div>
              <div style="font-size:12px;color:var(--ink4)">Muestra tu trabajo: eventos, montajes, presentaciones…</div>
              <button class="btn-primary" onclick="abrirModalPortafolio()" style="margin-top:12px;font-size:12px">📷
                Añadir primera foto</button>
            </div>
          <?php endif; ?>
          <?php if ($tipoServicio || $generos): ?>
            <div style="padding:14px 20px;border-top:1px solid var(--border)">
              <div
                style="font-size:11px;font-weight:700;color:var(--ink4);text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px">
              ...
              <div class="svc-chips" style="padding:0">
                ...
                <?php if ($generos):
                  foreach (explode(',', $talento['generos']) as $g) {
                    $g = trim($g);
                    if ($g)
                      echo "<span class='svc-chip'>🎶 " . htmlspecialchars($g) . "</span>";
                  }
                endif; ?>
      ...
      <div class="col-4">
        <div class="card">
          ...
          <div class="activity-list">
            ...
            <div class="act-item">
              <div class="act-dot"></div>
              <div class="act-txt"><strong><?= $vistasTotal ?> visitas
                  totales</strong><br><?= $vistas7dias > 0 ? '+' . $vistas7dias . ' esta semana' : 'Sin visitas recientes' ?></div>
            </div>
            <?php if ($galeriaTotal > 0): ?>
              <div class="act-item">
                <div class="act-dot"></div>
      ...
      <div class="col-4">
        <div class="card">
          <div class="card-header">
            <div class="card-title">🏆 Mis badges</div>
          </div>
          <div class="card-body">
            <div style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:14px">
              ...
              <?php if (!$tieneVerificado && !$tienePremium && !$tieneDestacado && !$tieneTop): ?>
                <div style="font-size:12.5px;color:var(--ink3)">Sin badges aún. <a href="verificar_cuenta.php"
                    style="color:var(--brand);font-weight:700">Verifica tu perfil →</a></div>
              <?php endif; ?>
            </div>
            <div style="background:var(--brand-light);border-radius:10px;padding:13px;border:1px solid var(--border)">
              <div style="font-size:11.5px;font-weight:800;color:var(--brand);margin-bottom:5px">¿Más contrataciones?
              </div>
              <div style="font-size:11.5px;color:var(--ink3);line-height:1.5">Badges <strong>Premium</strong> y
                <strong>Destacado</strong> desde el plan <strong>Verde Selva</strong>.</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="plan-grid">
              <?php foreach (['mensajes' => ['💬', 'Mensajes'], 'vacantes' => ['💼', 'Vacantes']] as $key => [$ico, $lab]):
                ...
                <div class="plan-bar">
                  ...
                  <div class="pb-track">
                    <div class="pb-fill <?= $cls ?>" style="width:<?= $pctB ?>%"></div>
                  </div>
                  <?php if (!$inf && $pctB >= 70): ?>
                    <div class="pb-warn"><?= $pctB >= 90 ? '⚠️ Límite' : '⚡ Casi al límite' ?></div><?php endif; ?>
                </div>
              <?php endforeach; ?>
            </div>
          </div>
  ...
  <div id="cropBannerModal"
    ...
    <div
      ...
      <div
        ...
        <img id="cropBannerImg" style="max-width:100%;display:block"></div>
      ...
      <div style="display:flex;gap:9px;margin-top:4px">
        ...
        <button onclick="confirmarCropBanner()" id="btnConfirmarCropBanner"
          style="flex:2;padding:10px;border-radius:9px;border:none;background:var(--brand);color:#fff;font-size:12.5px;font-weight:800;cursor:pointer">✅
          Usar banner</button>
      </div>
    </div>
  </div>
  ...
  <div class="modal-ov" id="modalPortafolio">
    <div class="modal-box">
      ...
      <div class="modal-pad">
        ...
        <p class="msub">Añade fotos de tus presentaciones, eventos y trabajos realizados. Máx. 15 fotos en plan básico.
        </p>
        <div id="portMsg" class="mmsg"></div>
        <div class="mfila">
          <div class="mgr full"><label>Título</label><input type="text" id="portTitulo"
              placeholder="ej: Presentación boda, DJ set festival…"></div>
          <div class="mgr full"><label>Descripción (opcional)</label><textarea id="portDesc" rows="2"
              placeholder="Cuéntanos sobre este trabajo…"></textarea></div>
        </div>
        <div
          style="border:2px dashed var(--border2);border-radius:12px;padding:24px;text-align:center;cursor:pointer;background:var(--brand-light);transition:.2s"
          onclick="document.getElementById('portFile').click()" onmouseover="this.style.background='var(--surface3)'"
        ...
        <input type="file" id="portFile" accept="image/jpeg,image/png,image/webp" style="display:none"
    ...
    <div class="modal-box">
      ...
      <div class="modal-pad">
        ...
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">
          <div id="fotoPreview"
            style="width:68px;height:68px;border-radius:50%;background:linear-gradient(135deg,#6d28d9,#8b5cf6);display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:900;color:white;overflow:hidden;flex-shrink:0;cursor:pointer"
            onclick="document.getElementById('fotoInput').click()">
            <?php if ($fotoUrl): ?><img src="<?= $fotoUrl ?>"
                style="width:100%;height:100%;object-fit:cover;border-radius:50%"><?php else: ?><?= $inicial ?><?php endif; ?>
          </div>
          <div>
            ...
            <div style="display:flex;gap:7px;flex-wrap:wrap">
              <button onclick="document.getElementById('fotoInput').click()"
                style="padding:7px 13px;border-radius:8px;background:var(--brand);color:white;border:none;font-size:12.5px;font-weight:700;cursor:pointer">📷
                Cambiar foto</button>
              <?php if ($fotoUrl): ?><button onclick="eliminarFoto()"
        ...
        <div class="mfila">
          <div class="mgr"><label>Nombre *</label><input type="text" id="editNombre"
              value="<?= htmlspecialchars($uRe['nombre'] ?? '') ?>"></div>
          <div class="mgr"><label>Apellido</label><input type="text" id="editApellido"
              value="<?= htmlspecialchars($uRe['apellido'] ?? '') ?>"></div>
        </div>
        <div class="mfila">
          <div class="mgr"><label>Ciudad</label><input type="text" id="editCiudad" value="<?= $ciudad ?>"
              placeholder="Quibdó"></div>
          <div class="mgr"><label>Teléfono / WhatsApp</label><input type="tel" id="editTelefono" value="<?= $telefono ?>"
              placeholder="3001234567"></div>
        </div>
        ...
        <div class="mfila">
          ...
          <div class="mgr"><label>Géneros / Estilos</label><input type="text" id="editGeneros"
              value="<?= htmlspecialchars($talento['generos'] ?? '') ?>" placeholder="Salsa, Vallenato, Electrónica…"></div>
        </div>
        <div class="mfila">
          <div class="mgr"><label>Precio desde (COP)</label><input type="number" id="editPrecio"
              value="<?= htmlspecialchars($talento['precio_desde'] ?? '') ?>" placeholder="150000"></div>
          <div class="mgr"><label>Disponibilidad</label><input type="text" id="editDisponib" value="<?= $disponib ?>"
              placeholder="Fines de semana…"></div>
        </div>
        <div class="mfila">
          <div class="mgr"><label>Instagram</label><input type="text" id="editInstagram"
              value="<?= htmlspecialchars($talento['instagram'] ?? '') ?>" placeholder="@miservicio"></div>
          <div class="mgr"><label>Habilidades / Skills</label><input type="text" id="editSkills"
              value="<?= htmlspecialchars($talento['skills'] ?? '') ?>" placeholder="Mezcla, producción, edición…"></div>
        </div>
        <div class="mfila">
          <div class="mgr full"><label>Sobre mí / Descripción</label><textarea id="editBio" rows="3"
              placeholder="Cuéntanos tu experiencia, equipos que usas, tipos de eventos que cubres…"><?= htmlspecialchars($talento['bio'] ?? '') ?></textarea>
          </div>
        </div>
  ...
  <div id="modalEliminarCuenta"
    ...
    <div
      ...
      <div style="text-align:center;margin-bottom:20px">
        ...
        <p style="font-size:13px;color:rgba(255,255,255,.6);line-height:1.6">Se eliminarán tu perfil, portafolio,
          mensajes y todos los datos asociados.</p>
      </div>
  ...
  <script>
    ...
    async function subirFoto(input) {
      ...
      try {
        const r = await fetch('dashboard_servicios.php', { method: 'POST', body: fd }); const j = await r.json();
        if (j.ok) {
