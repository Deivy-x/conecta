<?php
// ============================================================
// servicios.php — Servicios para Eventos del Chocó
// Migración exacta de la sección servicios-eventos del index.html
// Lee directamente de talento_perfil (precio_desde, generos, tipo_servicio)
// BUILD: v20260320
// ============================================================
...
// ── Emoji por tipo de servicio ───────────────────────────────
function emojiServicio($tipo, $profesion = '') {
  $t = strtolower($tipo . ' ' . $profesion);
  if (preg_match('/(dj|disc jockey|música|musica|champeta|salsa|afrobeat|reggaeton|electrónica)/i', $t)) return '🎧';
  if (preg_match('/(fotograf|video|cinéma|cineasta|audiovisual|produccion)/i', $t))                    return '📸';
  if (preg_match('/(chirimía|chirimia|marimba|tambor|trompeta|banda|grupo musical|conjunto)/i', $t))   return '🎺';
  if (preg_match('/(catering|gastronomia|gastronomía|cocinero|chef|pastelero|repostero)/i', $t))       return '🍽️';
  if (preg_match('/(decoracion|decoración|florista|ambientacion|montaje)/i', $t))                      return '🌸';
  if (preg_match('/(maestro.*ceremonia|animador|presentador|locutor|mc\b)/i', $t))                     return '🎤';
  if (preg_match('/(iluminacion|iluminación|sonido|tecnico.*sonido|audio)/i', $t))                     return '💡';
  if (preg_match('/(maquillaje|maquillador|estilista|peinado)/i', $t))                                 return '💄';
  if (preg_match('/(transporte|carro|van|bus|vehiculo|vehículo)/i', $t))                               return '🚐';
  if (preg_match('/(seguridad|vigilancia|guardia)/i', $t))                                             return '🛡️';
  return '🎵';
}
...
function badgeCategoria($tipo, $profesion = '') {
  $t = strtolower($tipo . ' ' . $profesion);
  if (preg_match('/(dj|música|musica|champeta|salsa|afrobeat|reggaeton|electrónica)/i', $t))          return ['🎵 Música', 'dorado'];
  if (preg_match('/(fotograf|video|cinéma|audiovisual)/i', $t))                                       return ['📸 Foto & Video', 'azul'];
  if (preg_match('/(chirimía|chirimia|marimba|banda|grupo musical)/i', $t))                           return ['🎺 Cultural', 'verde'];
  if (preg_match('/(catering|gastronomia|cocinero|chef)/i', $t))                                      return ['🍽️ Gastronomía', 'tierra'];
  if (preg_match('/(decoracion|florista|ambientacion)/i', $t))                                        return ['🌸 Decoración', 'rosa'];
  if (preg_match('/(maestro.*ceremonia|animador|presentador|locutor)/i', $t))                         return ['🎤 Animación', 'morado'];
...
// ── Unidad de precio por tipo ────────────────────────────────
function unidadPrecio($tipo, $profesion = '') {
  $t = strtolower($tipo . ' ' . $profesion);
  if (preg_match('/(catering|comida|chef|cocinero)/i', $t)) return '/persona';
  if (preg_match('/(fotograf|video)/i', $t))               return '/evento';
  if (preg_match('/(chirimia|chirimía|banda|grupo)/i', $t))return '/presentación';
  return '/evento';
}
...
if (file_exists(__DIR__ . '/Php/db.php')) {
  try {
    ...
    // Igual que talentos.php: MAX(id) + tiene precio o tipo_servicio = ofrece servicios
    // Fallback: si precio_desde no existe aún, muestra talentos con tipo_servicio definido
    try {
      $stmt = $db->query("
      ...
      // Si precio_desde no existe, query sin ese filtro
      $stmt = $db->query("
        SELECT u.id, u.nombre, u.apellido, u.ciudad, u.foto,
               u.verificado, u.badges_custom,
               tp.profesion, tp.bio, tp.skills, tp.generos,
    ...
    // Badges — idéntico a talentos.php
...
foreach ($dbServicios as $s) {
  ...
  if      (preg_match('/(dj|champeta|salsa|música|musica|afrobeat|reggaeton)/i',$t)) $catCounts['musica']++;
  elseif  (preg_match('/(fotograf|video|audiovisual)/i',$t))                         $catCounts['foto']++;
  elseif  (preg_match('/(chirimía|chirimia|marimba|banda|grupo musical)/i',$t))      $catCounts['cultural']++;
  elseif  (preg_match('/(catering|gastronomia|chef|cocinero)/i',$t))                 $catCounts['gastronomia']++;
  elseif  (preg_match('/(decoracion|florista|ambientacion)/i',$t))                   $catCounts['decoracion']++;
  elseif  (preg_match('/(animador|maestro.*ceremonia|locutor)/i',$t))                $catCounts['animacion']++;
  else                                                                                $catCounts['otros']++;
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <style>
    :root {
      --verde:    #1f9d55; --verde2:   #2ecc71; --verde-o:  #edfaf3;
      --dorado:   #d4a017; --dorado2:  #f0c040; --dorado-o: #fef9e7;
      --azul:     #1a3a6b; --azul2:    #2563eb;
      --oscuro:   #0a0f1e; --gris:     #f4f6f8;
      --texto:    #111;
      --choco-selva: #0a3320; --choco-tierra: #6d4c2a;
      --choco-flor:  #c0392b; --choco-rio:    #1a5276;
    }
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ...
    .nav-center a::after{content:"";position:absolute;left:0;bottom:-6px;width:0%;height:2px;background:var(--dorado);transition:width .3s}
    .nav-center a:hover::after,.nav-center a.active::after{width:100%}
    .nav-center .highlight{background:linear-gradient(135deg,var(--dorado),var(--dorado2));color:#111!important;padding:9px 20px;border-radius:25px;font-weight:700;box-shadow:0 4px 12px rgba(212,160,23,.35)}
    ...
    /* ── HERO — idéntico al servicios-eventos del index ── */
    .hero-servicios {
    ...
    .section-label-hero{
      ...
      background:rgba(212,160,23,.18);border:1px solid rgba(240,192,64,.35);
      color:var(--dorado2);font-size:12px;font-weight:800;
      padding:6px 18px;border-radius:30px;margin-bottom:20px;letter-spacing:.8px;text-transform:uppercase;
    }
    .hero-servicios h1{font-family:'Syne',sans-serif;font-size:62px;font-weight:900;line-height:1.05;margin-bottom:16px}
    .hero-servicios h1 .acento{color:var(--dorado2)}
    .hero-servicios .sub{font-size:18px;color:rgba(255,255,255,.7);margin-bottom:44px;line-height:1.6;max-width:580px}

    /* Buscador */
    .serv-search{display:flex;align-items:center;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);border-radius:50px;padding:6px;max-width:640px;margin-bottom:36px;backdrop-filter:blur(8px)}
    .serv-search .sf{display:flex;align-items:center;gap:10px;padding:10px 18px;flex:1}
    .serv-search .sf .icon{font-size:17px;opacity:.6}
    .serv-search input{border:none;outline:none;font-size:15px;width:100%;font-family:'DM Sans',sans-serif;color:white;background:transparent}
    .serv-search input::placeholder{color:rgba(255,255,255,.45)}
    .serv-search .divider{width:1px;height:30px;background:rgba(255,255,255,.2);flex-shrink:0}
    .serv-search button{background:linear-gradient(135deg,var(--dorado),var(--dorado2));color:#111;border:none;border-radius:40px;padding:12px 26px;font-size:14px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s;white-space:nowrap}
    .serv-search button:hover{opacity:.9;transform:scale(1.03)}
    ...
    .hcat{
      ...
      padding:8px 16px;border-radius:25px;cursor:pointer;transition:all .22s;
      font-family:'DM Sans',sans-serif;
    }
    .hcat:hover,.hcat.activo{background:rgba(212,160,23,.22);border-color:rgba(240,192,64,.4);color:var(--dorado2)}

    /* ── STATS BAND — fondo selva ── */
    .stats-band{
    ...
    .stats-band .s h3{font-family:'Syne',sans-serif;font-size:36px;font-weight:900;color:var(--dorado2)}
    .stats-band .s p{font-size:12px;color:rgba(255,255,255,.55);margin-top:5px;font-weight:600;text-transform:uppercase;letter-spacing:.5px}
    ...
    .filtros-inner{display:flex;align-items:center;gap:8px;overflow-x:auto;padding:18px 0;scrollbar-width:none}
    .filtros-inner::-webkit-scrollbar{display:none}
    .filtro-btn{
      display:flex;align-items:center;gap:6px;
      white-space:nowrap;padding:9px 18px;border-radius:25px;
      font-size:13px;font-weight:700;cursor:pointer;
      border:2px solid rgba(0,0,0,.08);background:white;color:#555;
      font-family:'DM Sans',sans-serif;transition:all .22s;flex-shrink:0;
    }
    ...
    .filtro-count{font-size:11px;background:rgba(0,0,0,.07);border-radius:10px;padding:1px 7px;font-weight:600}
    .filtro-btn.activo .filtro-count{background:rgba(0,0,0,.15)}
    ...
    .servicios-overlay{position:absolute;inset:0;background:linear-gradient(160deg,rgba(13,31,13,.93) 0%,rgba(10,26,58,.91) 100%)}
    .servicios-inner{position:relative;z-index:2}
    .servicios-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:32px;flex-wrap:wrap;gap:12px}
    .servicios-header h2{font-family:'Syne',sans-serif;font-size:32px;font-weight:800;color:white}
    .res-count{font-size:14px;color:rgba(255,255,255,.5);font-weight:500}
    ...
    /* ── EVENTO CARD — idéntica al index.html ── */
    .evento-card{
      background:rgba(255,255,255,.07);
      border:1px solid rgba(255,255,255,.12);
      border-radius:20px;padding:28px;
      transition:all .35s;backdrop-filter:blur(6px);
      cursor:pointer;
    }
    .evento-card:hover{
    ...
    .ev-icon{font-size:42px;line-height:1}
    .ev-badge{
      font-size:11px;font-weight:700;padding:4px 12px;border-radius:20px;
      background:rgba(212,160,23,.2);color:var(--dorado2);
      border:1px solid rgba(212,160,23,.3);white-space:nowrap;
    }
    .ev-badge.verde{background:rgba(31,157,85,.2);color:#4ade80;border-color:rgba(46,204,113,.3)}
    .ev-badge.azul{background:rgba(37,99,235,.2);color:#93c5fd;border-color:rgba(59,130,246,.3)}
    .ev-badge.morado{background:rgba(139,92,246,.2);color:#c4b5fd;border-color:rgba(167,139,250,.3)}
    .ev-badge.tierra{background:rgba(180,83,9,.2);color:#fcd34d;border-color:rgba(217,119,6,.3)}
    .ev-badge.rosa{background:rgba(236,72,153,.2);color:#f9a8d4;border-color:rgba(244,114,182,.3)}
    .ev-badge.rojo{background:rgba(239,68,68,.2);color:#fca5a5;border-color:rgba(248,113,113,.3)}
    .ev-estrellas{color:var(--dorado2);font-size:14px;margin-bottom:8px;letter-spacing:1px}
    .evento-card h3{font-size:18px;font-weight:800;margin-bottom:5px;color:white;line-height:1.2}
    .ev-nombre{color:var(--dorado2);font-weight:700;font-size:14px;margin-bottom:5px}
    .ev-lugar{font-size:13px;color:rgba(255,255,255,.5);margin-bottom:6px}
    .ev-precio{font-size:14px;color:var(--dorado2);font-weight:800;margin-bottom:6px}
    .ev-generos{font-size:12px;color:rgba(255,255,255,.4);margin-bottom:16px;line-height:1.5}
    .ev-btn{
      display:inline-block;color:var(--dorado2);font-weight:700;font-size:14px;
      text-decoration:none;border-bottom:2px solid transparent;transition:border-color .2s;
    }
    .ev-btn:hover{border-color:var(--dorado2)}

    /* ── SIN RESULTADOS ── */
    .no-results{grid-column:1/-1;text-align:center;padding:70px 20px;color:rgba(255,255,255,.5)}

    /* ── MODAL SERVICIO ── */
    .modal-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:2000;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(4px)}
    .modal-overlay.open{display:flex}
    .modal-box{
      background:#1a2635;border:1px solid rgba(255,255,255,.1);
      border-radius:24px;max-width:560px;width:100%;
      box-shadow:0 30px 80px rgba(0,0,0,.5);animation:fadeUp .3s ease both;
      position:relative;max-height:90vh;overflow-y:auto;
    }
    @keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
    .modal-close{position:absolute;top:18px;right:20px;background:none;border:none;font-size:22px;cursor:pointer;color:rgba(255,255,255,.5)}
    .modal-close:hover{color:white}
    .modal-header{
      padding:36px 36px 24px;display:flex;gap:20px;align-items:flex-start;
    ...
    .modal-info h2{font-family:'Syne',sans-serif;font-size:22px;font-weight:800;color:white;margin-bottom:4px}
    .modal-info .m-nombre{color:var(--dorado2);font-weight:700;font-size:14px;margin-bottom:4px}
    .modal-info .m-lugar{color:rgba(255,255,255,.5);font-size:13px}
    .modal-body{padding:24px 36px 36px}
    .modal-precio{font-size:20px;font-weight:900;color:var(--dorado2);margin-bottom:12px}
    .modal-estrellas{color:var(--dorado2);font-size:15px;margin-bottom:16px;letter-spacing:1px}
    .modal-desc{font-size:14px;color:rgba(255,255,255,.65);line-height:1.7;margin-bottom:20px}
    .modal-generos-titulo{font-size:11px;font-weight:800;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px}
    .modal-generos-tags{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:24px}
    .modal-gtag{
      background:rgba(212,160,23,.12);border:1px solid rgba(212,160,23,.2);
      color:rgba(255,255,255,.7);font-size:12px;font-weight:600;padding:5px 14px;border-radius:20px;
    }
    .modal-btn-contratar{
      display:flex;align-items:center;justify-content:center;gap:10px;
      width:100%;padding:15px;
      background:linear-gradient(135deg,var(--dorado),var(--dorado2));
      color:#111;border:none;border-radius:14px;font-size:15px;font-weight:800;
      font-family:'DM Sans',sans-serif;cursor:pointer;text-decoration:none;
      box-shadow:0 6px 20px rgba(212,160,23,.45);transition:transform .2s;
    }
    .modal-btn-contratar:hover{transform:translateY(-2px)}
    .modal-btn-chat{
      display:flex;align-items:center;justify-content:center;gap:10px;
      width:100%;padding:13px;margin-top:10px;
      background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.14);
      color:rgba(255,255,255,.8);border-radius:14px;font-size:14px;font-weight:700;
      font-family:'DM Sans',sans-serif;cursor:pointer;text-decoration:none;transition:all .2s;
    }
    .modal-btn-chat:hover{background:rgba(255,255,255,.14);color:white}

    /* ── CTA REGISTRO — idéntico al index ── */
    .ev-cta-section{
    ...
    .ev-cta-section h2{font-family:'Syne',sans-serif;font-size:38px;font-weight:900;color:white;margin-bottom:14px}
    .ev-cta-section p{font-size:16px;color:rgba(255,255,255,.65);max-width:520px;margin:0 auto 40px;line-height:1.65}
    .btn-dorado{
      display:inline-block;
      background:linear-gradient(135deg,var(--dorado),var(--dorado2));
      color:#111;padding:16px 44px;border-radius:30px;font-weight:800;font-size:16px;
      text-decoration:none;box-shadow:0 8px 28px rgba(212,160,23,.4);transition:transform .2s;
    }
    .btn-dorado:hover{transform:translateY(-3px)}
    .btn-outline-w{
      display:inline-block;
      border:2px solid rgba(255,255,255,.3);color:rgba(255,255,255,.8);
      padding:15px 32px;border-radius:30px;font-weight:600;font-size:15px;
      text-decoration:none;margin-left:14px;transition:all .2s;
    }
    .btn-outline-w:hover{border-color:var(--dorado2);color:var(--dorado2)}

    /* ── FOOTER ── */
    footer{background:#0a0f1e;border-top:1px solid rgba(255,255,255,.06);color:rgba(255,255,255,.5);text-align:center;padding:28px 48px;font-size:14px}
    ...
    /* ── RESPONSIVE ── */
    @media(max-width:1200px){.hero-servicios h1{font-size:48px}.navbar{padding:0 32px}}
    @media(max-width:900px){.hero-servicios{padding:120px 32px 80px}.hero-servicios h1{font-size:38px}}
    @media(max-width:768px){
      .navbar{padding:0 20px}.nav-center,.nav-right{display:none}.hamburger{display:flex}
      .hero-servicios{padding:110px 20px 70px}.hero-servicios h1{font-size:28px;line-height:1.15}
      .serv-search{flex-wrap:wrap;border-radius:18px;padding:10px}
      .serv-search .sf{width:100%}.serv-search .divider{width:100%;height:1px}
      .serv-search button{width:100%;border-radius:12px}
      .stats-band{padding:36px 20px}
      .filtros-section{padding:0 20px}
      .servicios-section{padding:50px 20px 70px}
      .eventos-grid{grid-template-columns:1fr}
      .ev-cta-section{padding:60px 20px}
      .modal-header{flex-direction:column;gap:14px}
      .btn-outline-w{margin:12px 0 0;display:block;text-align:center}
    }
    @media(max-width:480px){
      .hero-servicios h1{font-size:24px}
      .stats-band{grid-template-columns:1fr 1fr}
      .ev-cta-section h2{font-size:28px}
    }
  </style>
</head>
<body>
...
<!-- ── HERO — idéntico a servicios-eventos del index ── -->
<section class="hero-servicios">
  ...
  <div class="hero-inner">
    ...
    <p class="sub">DJs, artistas, fotógrafos, chirimías, catering y más — con acuerdo de pago seguro para Quibdó y el Chocó.</p>
...
<div class="filtros-section">
  <div class="filtros-inner">
    <button class="filtro-btn activo" data-cat="todos"      onclick="filtrarCat('todos',this)">🌐 Todos <span class="filtro-count"><?= $totalAll ?></span></button>
    <button class="filtro-btn"        data-cat="musica"     onclick="filtrarCat('musica',this)">🎧 DJs & Música <span class="filtro-count"><?= $catCounts['musica'] ?></span></button>
    <button class="filtro-btn"        data-cat="foto"       onclick="filtrarCat('foto',this)">📸 Foto & Video <span class="filtro-count"><?= $catCounts['foto'] ?></span></button>
    <button class="filtro-btn"        data-cat="cultural"   onclick="filtrarCat('cultural',this)">🎺 Chirimía & Cultural <span class="filtro-count"><?= $catCounts['cultural'] ?></span></button>
    <button class="filtro-btn"        data-cat="gastronomia" onclick="filtrarCat('gastronomia',this)">🍽️ Catering <span class="filtro-count"><?= $catCounts['gastronomia'] ?></span></button>
    <button class="filtro-btn"        data-cat="decoracion" onclick="filtrarCat('decoracion',this)">🌸 Decoración <span class="filtro-count"><?= $catCounts['decoracion'] ?></span></button>
    <button class="filtro-btn"        data-cat="animacion"  onclick="filtrarCat('animacion',this)">🎤 Animación <span class="filtro-count"><?= $catCounts['animacion'] ?></span></button>
  </div>
</div>

<!-- ── GRID — diseño idéntico al index.html ── -->
    ...
    <div class="eventos-grid" id="serviciosGrid">
      ...
      <?php if (!empty($dbServicios)): ?>
        <?php foreach ($dbServicios as $srv):
          ...
          // Override badge si tiene insignia especial
          if ($srv['tiene_top'])        { $badgeTxt = '👑 Top';       $badgeCls = 'rojo'; }
          elseif ($srv['tiene_premium']){ $badgeTxt = '⭐ Premium';    $badgeCls = 'dorado'; }
          elseif ($srv['tiene_destacado']){ $badgeTxt = '🏅 Destacado'; $badgeCls = 'morado'; }
          elseif ($srv['tiene_verificado']){ $badgeTxt = '⭐ Verificado'; $badgeCls = 'dorado'; }

          // Categoría para filtro JS
          $t = strtolower($tipo.' '.$srv['profesion']);
          if      (preg_match('/(dj|champeta|salsa|música|musica|afrobeat|reggaeton|electrónica)/i',$t)) $catData = 'musica';
          elseif  (preg_match('/(fotograf|video|audiovisual)/i',$t))                                      $catData = 'foto';
          elseif  (preg_match('/(chirimía|chirimia|marimba|banda|grupo musical)/i',$t))                   $catData = 'cultural';
          elseif  (preg_match('/(catering|gastronomia|chef|cocinero)/i',$t))                              $catData = 'gastronomia';
          elseif  (preg_match('/(decoracion|florista|ambientacion)/i',$t))                                $catData = 'decoracion';
          elseif  (preg_match('/(animador|maestro.*ceremonia|locutor)/i',$t))                             $catData = 'animacion';
          else                                                                                             $catData = 'otros';
        ...
        ?>
          <div class="evento-card reveal"
               data-cat="<?= $catData ?>"
               data-uid="<?= $srv['id'] ?>"
               data-nombre="<?= $nb ?>" data-pro="<?= $pro ?>"
               data-ciu="<?= $ciu ?>" data-bio="<?= $bio ?>"
               data-generos="<?= $generos ?>" data-precio="<?= $precio ?>"
               data-unidad="<?= $unidad ?>" data-emoji="<?= $emoji ?>"
               data-stars="<?= $stars ?>" data-foto="<?= $foto ?>"
               data-grad="<?= $grd ?>" data-ini="<?= $ini ?>"
      ...
      <?php if (empty($dbServicios)): ?>
        <div class="no-results">
          ...
          <p style="font-size:16px;font-weight:700;color:rgba(255,255,255,.7);margin-bottom:8px">Aún no hay servicios registrados</p>
          <p style="font-size:14px;margin-bottom:24px">¡Sé el primero en ofrecer tu servicio para eventos!</p>
          <a href="registro.php" style="display:inline-block;padding:13px 32px;background:linear-gradient(135deg,var(--dorado),var(--dorado2));color:#111;border-radius:30px;text-decoration:none;font-weight:800">🎧 Registrar mi servicio</a>
        </div>
      <?php endif; ?>
...
<!-- CTA — idéntico al botón del index -->
<section class="ev-cta-section">
  <h2 class="reveal">¿Ofreces un servicio para eventos en el Chocó?</h2>
  <p>Regístrate gratis, configura tu perfil con tu precio y géneros, y empieza a recibir contratos con acuerdo de pago seguro.</p>
  ...
  <div class="modal-box" style="max-width:680px">
    ...
    <div class="modal-body">
      ...
      <div id="mGaleriaWrap" style="display:none;margin-bottom:20px">
        <div style="font-size:11px;font-weight:800;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px">📸 Galería de evidencias</div>
        <div id="mGaleriaGrid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px"></div>
      </div>
      <div id="mGaleriaLoading" style="display:none;text-align:center;padding:16px;color:rgba(255,255,255,.4);font-size:13px">⚙️ Cargando galería…</div>
...
<script>
  ...
  // ── FILTRAR por categoría — idéntico a filtrarLocales() ──
  let catActiva = 'todos';
  let textoBusq = '';

  function filtrarCat(cat, btn) {
    catActiva = cat;
    // Actualizar todos los botones (hero + filtros sticky)
