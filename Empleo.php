...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .nav-center{display:flex;align-items:center;gap:18px;flex:1;justify-content:center}
    .nav-center a{color:#333;text-decoration:none;font-size:14px;font-weight:500;padding:6px 4px;position:relative}
    ...
    .categorias-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:16px;max-width:1100px;margin:0 auto}
    .categoria-card{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;background:white;border:1.5px solid rgba(0,0,0,.07);border-radius:20px;padding:22px 16px 18px;font-size:13px;font-weight:700;color:#2a2a2a;cursor:pointer;transition:all .3s cubic-bezier(.34,1.56,.64,1);box-shadow:0 2px 12px rgba(0,0,0,.06);position:relative;overflow:hidden;font-family:'DM Sans',sans-serif}
    .categoria-card .cat-icon{font-size:30px;line-height:1;display:block;transition:transform .3s cubic-bezier(.34,1.56,.64,1)}
    .categoria-card .cat-label{font-size:12.5px;font-weight:700;letter-spacing:.01em;text-align:center;line-height:1.3}
    .categoria-card .count{font-size:11px;color:#aaa;font-weight:500;margin-top:2px}
    .categoria-card:hover,.categoria-card.activa{background:linear-gradient(135deg,#edfaf3,#d4f5e4);border-color:#1f9d55;color:#1f9d55;transform:translateY(-4px) scale(1.03);box-shadow:0 12px 28px rgba(31,157,85,.18)}
    .categoria-card.activa .count{color:#1f9d55}
    .categoria-card:hover .cat-icon,.categoria-card.activa .cat-icon{transform:scale(1.25) rotate(-5deg)}
...
<section class="hero">
  ...
  <section class="categorias">
    ...
    <div class="categorias-grid" id="catGrid">
      <button class="categoria-card activa" data-cat="todos"><span class="cat-icon">🌐</span><span class="cat-label">Todos</span><span class="count" id="cnt-todos"><?= $totalVacantes ?></span></button>
      <button class="categoria-card" data-cat="administrativo"><span class="cat-icon">💼</span><span class="cat-label">Administrativo</span><span class="count" id="cnt-adm">0</span></button>
      <button class="categoria-card" data-cat="tecnologia"><span class="cat-icon">💻</span><span class="cat-label">Tecnología</span><span class="count" id="cnt-tec">0</span></button>
      <button class="categoria-card" data-cat="arte"><span class="cat-icon">🎵</span><span class="cat-label">Arte &amp; Música</span><span class="count" id="cnt-art">0</span></button>
      <button class="categoria-card" data-cat="educacion"><span class="cat-icon">📚</span><span class="cat-label">Educación</span><span class="count" id="cnt-edu">0</span></button>
      <button class="categoria-card" data-cat="salud"><span class="cat-icon">🏥</span><span class="cat-label">Salud</span><span class="count" id="cnt-sal">0</span></button>
      <button class="categoria-card" data-cat="gastronomia"><span class="cat-icon">🍽️</span><span class="cat-label">Gastronomía</span><span class="count" id="cnt-gas">0</span></button>
      <button class="categoria-card" data-cat="tecnico"><span class="cat-icon">🔧</span><span class="cat-label">Técnico</span><span class="count" id="cnt-tec2">0</span></button>
      <button class="categoria-card" data-cat="transporte"><span class="cat-icon">🚗</span><span class="cat-label">Transporte</span><span class="count" id="cnt-tra">0</span></button>
      <button class="categoria-card" data-cat="construccion"><span class="cat-icon">🏗️</span><span class="cat-label">Construcción</span><span class="count" id="cnt-con">0</span></button>
    </div>
  </section>
...
<div id="qc-login-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:10000;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(8px)" onclick="if(event.target===this)cerrarModalLogin()">
  <div style="background:#fff;border-radius:24px;max-width:420px;width:100%;box-shadow:0 24px 60px rgba(0,0,0,.22);overflow:hidden;animation:qcLoginIn .35s cubic-bezier(.34,1.56,.64,1)">
    <div style="background:linear-gradient(135deg,#1f9d55,#2ecc71);padding:28px 28px 22px;text-align:center;position:relative">
      ...
      <p style="margin:8px 0 0;color:rgba(255,255,255,.88);font-size:14px;font-family:'DM Sans',sans-serif">Encuentra empleos y oportunidades en Quibdó</p>
    </div>
