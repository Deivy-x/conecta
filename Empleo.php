  ...
  <style>
    * {
    ...
    .hero {
      ...
      background: linear-gradient(135deg, #0b3a7e 0%, #0f172a 60%, #0b3a7e 100%);
      text-align: center;
      position: relative;
      overflow: hidden
    }
    ...
    .hero::before {
      ...
      position: absolute;
      inset: 0;
      background: radial-gradient(ellipse at 30% 50%, rgba(31, 157, 85, .12) 0%, transparent 60%), radial-gradient(ellipse at 70% 50%, rgba(11, 58, 126, .2) 0%, transparent 60%)
    }
    ...
    .categorias h2 {
      ...
      font-weight: 800;
      margin-bottom: 10px;
      background: linear-gradient(135deg, #1f9d55, #2563eb);
    ...
    .categoria-card.activa {
      background: linear-gradient(135deg, #edfaf3, #d4f5e4);
      border-color: #1f9d55;
      color: #1f9d55;
      transform: translateY(-4px) scale(1.03);
      box-shadow: 0 12px 28px rgba(31, 157, 85, .18)
    }

    .categoria-card.activa .count {
      color: #1f9d55
    }

    .categoria-card:hover .cat-icon,
    .categoria-card.activa .cat-icon {
    ...
    .modal-btn:hover {
      ...
      box-shadow: 0 10px 28px rgba(31, 157, 85, .45)
    }
...
<body>
  ...
  <section class="hero">
    <div class="hero-content reveal">
      ...
      <p class="hero-subtitle">Vacantes reales de empresas y negocios locales del Chocó. Encuentra tu próxima
      ...
      <div class="hero-stats">
        <div class="stat">
          <div class="stat-num" id="statVacantes"><?= $totalVacantes > 0 ? '+' . $totalVacantes : '+300' ?></div>
          <div class="stat-label">Vacantes activas</div>
        </div>
        <div class="stat">
          <div class="stat-num"><?= $totalEmpresas > 0 ? '+' . $totalEmpresas : '+120' ?></div>
  ...
  <section class="categorias">
    ...
    <div class="categorias-grid" id="catGrid">
      <button class="categoria-card activa" data-cat="todos"><span class="cat-icon">🌐</span><span
          class="cat-label">Todos</span><span class="count" id="cnt-todos"><?= $totalVacantes ?></span></button>
      <button class="categoria-card" data-cat="administrativo"><span class="cat-icon">💼</span><span
          class="cat-label">Administrativo</span><span class="count" id="cnt-adm">0</span></button>
      <button class="categoria-card" data-cat="tecnologia"><span class="cat-icon">💻</span><span
          class="cat-label">Tecnología</span><span class="count" id="cnt-tec">0</span></button>
      <button class="categoria-card" data-cat="arte"><span class="cat-icon">🎵</span><span class="cat-label">Arte &amp;
          Música</span><span class="count" id="cnt-art">0</span></button>
      <button class="categoria-card" data-cat="educacion"><span class="cat-icon">📚</span><span
          class="cat-label">Educación</span><span class="count" id="cnt-edu">0</span></button>
      <button class="categoria-card" data-cat="salud"><span class="cat-icon">🏥</span><span
          class="cat-label">Salud</span><span class="count" id="cnt-sal">0</span></button>
      <button class="categoria-card" data-cat="gastronomia"><span class="cat-icon">🍽️</span><span
          class="cat-label">Gastronomía</span><span class="count" id="cnt-gas">0</span></button>
      <button class="categoria-card" data-cat="tecnico"><span class="cat-icon">🔧</span><span
          class="cat-label">Técnico</span><span class="count" id="cnt-tec2">0</span></button>
      <button class="categoria-card" data-cat="transporte"><span class="cat-icon">🚗</span><span
          class="cat-label">Transporte</span><span class="count" id="cnt-tra">0</span></button>
      <button class="categoria-card" data-cat="construccion"><span class="cat-icon">🏗️</span><span
          class="cat-label">Construcción</span><span class="count" id="cnt-con">0</span></button>
    </div>
  </section>
  ...
  <section class="empleos" id="sec-empleos">
    <div class="empleos-header">
      ...
      <span class="resultado-count" id="resCount"><?= $totalVacantes ?> vacante<?= $totalVacantes != 1 ? 's' : '' ?>
        encontrada<?= $totalVacantes != 1 ? 's' : '' ?></span>
    </div>
    ...
    <div class="empleos-grid" id="empleosGrid">
      <?php if (!empty($vacantesDB)): ?>
        <?php foreach ($vacantesDB as $v):
          ...
          ?>
          <div class="empleo-card reveal" data-empid="<?= (int) $v['id'] ?>"
            data-tipo="<?= htmlspecialchars($v['tipo'] ?? 'tiempo completo') ?>" data-cat="<?= $catFilter ?>"
            data-titulo="<?= $titulo ?>" data-empresa="<?= $empresa ?>" data-lugar="<?= $ciudad ?>"
            data-salario="<?= htmlspecialchars($salario) ?>" data-verificada="<?= $v['verificado'] ? '1' : '0' ?>"
            data-desc="<?= $desc ?>" data-modalidad="<?= $modalidad ?>">
            ...
            <div class="empleo-tags">
              ...
              <?php if ($v['categoria']): ?><span
                  class="tag"><?= htmlspecialchars(ucfirst($v['categoria'])) ?></span><?php endif; ?>
            </div>
    ...
    <div class="modal-box" id="modalBox">
      ...
      <div id="modalCabecera" style="display:flex;align-items:center;gap:16px;margin-bottom:18px">
        <div id="modalIconWrap"
          style="width:56px;height:56px;border-radius:14px;background:#edfaf3;display:flex;align-items:center;justify-content:center;font-size:28px;flex-shrink:0">
          💼</div>
      ...
      <div id="modalSolMsg"
        style="display:none;margin-top:10px;padding:10px 14px;border-radius:10px;font-size:13px;font-weight:600;text-align:center">
      </div>
