...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'editar_negocio') {
    ...
    if (!$nombreNeg) {
      echo json_encode(['ok' => false, 'msg' => 'El nombre del negocio es obligatorio.']);
      exit;
    }
  ...
  if ($action === 'toggle_visibilidad') {
    ...
    // ── Verificar plan Verde Selva antes de activar visibilidad en directorio ──
    if ($visible === 1 && function_exists('tieneBeneficio')) {
      if (!tieneBeneficio($db, $usuario['id'], 'portafolio')) {
        echo json_encode([
          'ok' => false,
          'msg' => '🌿 Necesitas el plan Verde Selva o superior para aparecer en el directorio de negocios.',
          'requiere_plan' => true
        ]);
        exit;
      }
  ...
  if ($action === 'subir_banner') {
    ...
    try {
      $db->exec("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS banner VARCHAR(500) DEFAULT '' AFTER foto");
    } catch (Exception $e) {
    }
    require_once __DIR__ . '/Php/cloudinary_upload.php';
  ...
  if ($action === 'subir_evidencia') {
    ...
    if (!in_array($ext, ['jpg', 'jpeg', 'png', 'webp'])) {
      echo json_encode(['ok' => false, 'msg' => 'Solo JPG/PNG/WEBP.']);
      exit;
    }
...
$uRe = $db->prepare("SELECT * FROM usuarios WHERE id=?");
$uRe->execute([$usuario['id']]);
$uRe = $uRe->fetch();
  ...
  <style>
    ...
    *::after {
    ...
    .sidebar {
      ...
      border-right: 2px solid rgba(180, 83, 9, .10);
      display: flex;
    ...
    .sidebar-plan {
      background: linear-gradient(135deg, #b45309 0%, #d97706 60%, #1a3a6b 100%);
      border-radius: var(--radius);
      padding: 14px;
      color: #fff;
      margin-bottom: 10px
    ...
    .topbar {
      ...
      border-bottom: 2px solid rgba(180, 83, 9, .12);
      position: fixed;
      top: 4px;
      left: var(--nav-w);
      right: 0;
    ...
    .tb-btn:focus-within .tb-notif-panel,
    .tb-btn.open .tb-notif-panel {
      display: block
    }
    ...
    /* ── HERO STRIP ── */
    .hero-strip {
      background: linear-gradient(135deg, #fff 0%, #fffbeb 60%, #f0fdf4 100%);
    ...
    .hero-strip::before {
      ...
      right: 0;
      height: 4px;
      background: linear-gradient(90deg, #d97706 33.3%, #1f9d55 33.3% 66.6%, #1a3a6b 66.6%)
    }
...
<body>
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
      <div class="nav-section">
        ...
        <a href="chat.php" class="nav-item"><span class="ni-ico">💬</span> Mensajes<?php if ($chatNoLeidos > 0): ?><span
              class="ni-badge"><?= $chatNoLeidos ?></span><?php endif; ?></a>
      </div>
      <div class="nav-section">
  ...
  <div class="topbar">
    <button class="hamburger" onclick="toggleSidebar()"
      aria-label="Menú"><span></span><span></span><span></span></button>
    ...
    <div class="topbar-actions">
      <button class="btn-primary" onclick="abrirModal()" style="display:flex;align-items:center;gap:5px">✏️ Editar
        negocio</button>
      <div style="position:relative">
        <div class="tb-btn" id="navNotif" title="Notificaciones">🔔<div class="tb-dot" id="notifDot"
      ...
      <div style="position:relative" id="dashUserWidget">
        ...
        <div class="dash-dropdown" id="dashDropdown">
          <div class="dash-drop-header">
            <div class="dash-drop-av" id="dropAv"><?php if ($fotoUrl): ?><img src="<?= $fotoUrl ?>"
                  alt="Logo"><?php else: ?><?= $inicial ?><?php endif; ?></div>
            <div>
          ...
          <?php if (!empty($badgesHTML)): ?>
            <div class="dash-drop-badges"><?= $badgesHTML ?></div><?php endif; ?>
          <div class="dash-drop-menu">
            <a href="dashboard_negocios.php" class="dash-drop-link"><span class="dash-dl-icon">🏠</span> Mi panel</a>
            <a href="chat.php" class="dash-drop-link"><?php if ($chatNoLeidos > 0): ?><span
                  class="dash-dl-badge"><?= $chatNoLeidos ?></span><?php endif; ?><span class="dash-dl-icon">💬</span>
              Mensajes</a>
            <a href="verificar_cuenta.php" class="dash-drop-link"><span class="dash-dl-icon">🪪</span> Verificación</a>
            <div class="dash-drop-sep"></div>
            <a href="Php/logout.php" class="dash-drop-link dash-drop-logout"><span class="dash-dl-icon">🚪</span> Cerrar
              sesión</a>
          </div>
        </div>
      </div>
  ...
  <main class="main">
    ...
    <?php if (!$tieneVerificado): ?>
      <?php if ($estadoVerif === 'pendiente'): ?>
        <div class="alert-bar ap">
          <div class="a-ico">⏳</div>
          <div class="a-txt"><strong>Documentos en revisión</strong><span>El administrador está revisando tu RUT o registro
              mercantil.</span></div>
        </div>
      <?php elseif ($estadoVerif === 'rechazado'): ?>
        <div class="alert-bar ar">
          <div class="a-ico">❌</div>
          <div class="a-txt"><strong>Verificación rechazada</strong><span>Sube los documentos con mejor calidad.</span>
          </div><a href="verificar_cuenta.php" class="a-btn">Reintentar</a>
        </div>
      <?php else: ?>
        <div class="alert-bar as">
          <div class="a-ico">🏪</div>
          <div class="a-txt"><strong>Verifica tu negocio</strong><span>Sube tu RUT o registro mercantil y obtén el badge de
              Negocio Verificado.</span></div><a href="verificar_cuenta.php" class="a-btn">Verificar</a>
        </div>
      <?php endif; ?>
    <?php else: ?>
      <div class="alert-bar av">
        <div class="a-ico">✅</div>
        <div class="a-txt"><strong>Negocio verificado</strong><span>Los clientes ven tu badge de Negocio
            Verificado.</span></div>
      </div>
    <?php endif; ?>
    ...
    <div class="hero-strip">
      ...
      <div class="hero-stats">
        <div class="hs">
          <div class="hs-val"><?= $vistasTotal ?></div>
          <div class="hs-lab">Visitas</div>
        </div>
        <div class="hs">
          <div class="hs-val"><?= $vistas7dias > 0 ? '+' . $vistas7dias : '0' ?></div>
          <div class="hs-lab">Esta semana</div>
        </div>
        <div class="hs">
    ...
    <div class="dashboard-grid">
        ...
        <div class="card" style="padding:0;overflow:hidden">
          ...
          <div class="banner-zone" id="bannerZone" onclick="document.getElementById('bannerInput').click()"
            ...
            <?php if ($bannerUrl): ?>
              <button onclick="event.stopPropagation();eliminarBanner()"
                style="position:absolute;top:8px;right:8px;background:rgba(0,0,0,.55);color:#fff;border:none;border-radius:20px;padding:4px 11px;font-size:11px;font-weight:700;cursor:pointer">🗑
                Quitar</button>
            <?php endif; ?>
          </div>
          ...
          <div class="logo-row">
            ...
            <div style="flex:1;min-width:120px;padding-top:46px">
              ...
              <div style="font-size:12.5px;color:var(--ink3);margin-top:2px" id="dCategoria">
                <?= $categoria ?: '' ?><?php if ($ciudad): ?> · 📍 <?= $ciudad ?><?php endif; ?></div>
            </div>
            <div style="display:flex;gap:8px;padding-top:46px;flex-wrap:wrap">
              <a href="perfil.php?id=<?= $usuario['id'] ?>" target="_blank" class="btn-secondary"
                style="font-size:12px;padding:7px 13px">👁 Ver perfil</a>
              <button onclick="abrirModal()" class="btn-primary" style="font-size:12px;padding:7px 13px">✏️
                Editar</button>
            </div>
          </div>
      ...
      <div class="col-4">
        <div class="card" style="height:100%">
          ...
          <div class="profile-card">
            ...
            <div class="pc-info">
              <?php if ($ciudad): ?>
                <div class="pc-row"><span class="pc-ico">📍</span><span><?= $ciudad ?></span></div><?php endif; ?>
              <?php if ($direccion): ?>
                <div class="pc-row"><span class="pc-ico">🗺️</span><span><?= $direccion ?></span></div><?php endif; ?>
              <?php if ($whatsapp): ?>
                <div class="pc-row"><span class="pc-ico">💬</span><span><?= $whatsapp ?></span></div><?php endif; ?>
              <?php if ($horario): ?>
                <div class="pc-row"><span class="pc-ico">🕐</span><span><?= $horario ?></span></div><?php endif; ?>
              <div class="pc-row"><span class="pc-ico">✉️</span><span><?= $correo ?></span></div>
              <?php if (!empty($badgesHTML)): ?>
                <div style="margin-top:8px"><?= $badgesHTML ?></div><?php endif; ?>
            </div>
          </div>
          <div class="vis-row" style="margin:0 20px 12px">
            <div>
              <div class="vis-lab">Visible en directorio</div>
              <div class="vis-sub">Aparece en Negocios públicos</div>
            </div>
        ...
        <div class="card">
          ...
          <div class="actions-grid">
            <a href="chat.php" class="action-btn">
              ...
              <div class="ab-label">Mensajes</div>
              <?php if ($chatNoLeidos > 0): ?><span class="ab-badge"><?= $chatNoLeidos ?></span><?php else: ?>
                <div class="ab-desc">Sin nuevos</div><?php endif; ?>
            </a>
            <a href="negocios.php" class="action-btn">
              <div class="ab-ico">🏪</div>
              <div class="ab-label">Ver directorio</div>
              <div class="ab-desc">Negocios del Chocó</div>
            </a>
            <a href="javascript:void(0)" onclick="abrirModal()" class="action-btn">
              <div class="ab-ico">✏️</div>
              <div class="ab-label">Editar negocio</div>
              <div class="ab-desc">Actualizar datos</div>
            </a>
            <a href="javascript:void(0)" onclick="abrirModalGaleria()" class="action-btn">
              <div class="ab-ico">📸</div>
              <div class="ab-label">Galería</div>
              <div class="ab-desc"><?= $galeriaTotal ?> fotos</div>
            </a>
            <a href="verificar_cuenta.php" class="action-btn">
              <div class="ab-ico">🪪</div>
              <div class="ab-label">Verificación</div>
              <div class="ab-desc"><?= $tieneVerificado ? '✅ Verificado' : 'Subir docs' ?></div>
            </a>
            <a href="buscar.php" class="action-btn">
              <div class="ab-ico">🔍</div>
              <div class="ab-label">Buscar</div>
              <div class="ab-desc">Talentos / empresas</div>
            </a>
            <a href="Ayuda.html" class="action-btn">
              <div class="ab-ico">❓</div>
              <div class="ab-label">Ayuda</div>
              <div class="ab-desc">Soporte y guías</div>
            </a>
          </div>
        </div>
      </div>
      ...
      <div class="col-8">
        <div class="card">
          <div class="card-header">
            <div class="card-title">📸 Galería del negocio <span
                style="background:var(--surface3);color:var(--brand);font-size:10px;padding:2px 8px;border-radius:20px;font-weight:700;margin-left:6px"><?= $galeriaTotal ?>
                fotos</span></div>
            <button onclick="abrirModalGaleria()" class="btn-primary" style="font-size:12px;padding:6px 13px">+
              Añadir</button>
          </div>
          <div class="galeria-grid" id="galeriaGrid">
            <?php foreach ($galeriaItems as $g): ?>
              <div class="gal-item">
                <?php if ($g['tipo'] === 'video'): ?>
                  <div
                    style="width:100%;height:100%;background:#111;display:flex;align-items:center;justify-content:center;font-size:28px">
                    ▶️</div>
                <?php else: ?>
      ...
      <div class="col-4" style="display:flex;flex-direction:column;gap:18px">
        ...
        <div class="card">
          ...
          <div class="card-body" style="padding-top:12px;display:flex;flex-direction:column">
            ...
            <div class="activity-item">
              <div class="act-dot-w"></div>
              <div class="act-body"><?= $vistasTotal ?> visitas
                acumuladas<?= $vistas7dias > 0 ? ' · +' . $vistas7dias . ' esta semana' : '' ?></div>
          ...
          <div class="card" style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border:1.5px solid #fcd34d">
            <div class="card-header" style="padding:16px 18px 0">
              <div style="display:flex;align-items:center;gap:10px">
                <div
                  style="width:40px;height:40px;border-radius:12px;background:var(--brand);display:flex;align-items:center;justify-content:center;font-size:18px">
                  ⭐</div>
                <div>
                  <div
                    style="font-size:10px;font-weight:700;color:var(--brand);text-transform:uppercase;letter-spacing:1px">
                    Plan activo</div>
                  <div style="font-size:18px;font-weight:800;color:var(--ink)">
                    <?= htmlspecialchars($datosPlan['nombre'] ?? 'Semilla') ?></div>
                </div>
              </div>
              ...
              <?php foreach (['mensajes' => ['💬', 'Mensajes'], 'vacantes' => ['💼', 'Vacantes']] as $key => [$ico, $lab]):
                ...
                <div class="plan-bar-item">
                  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
                    ...
                    <span style="font-size:13px;font-weight:800;color:var(--ink)"><?= $uso ?><span
                        style="font-size:10px;font-weight:400;color:var(--ink4)"> / <?= $inf ? '∞' : $lim ?></span></span>
                  </div>
                  <div style="height:5px;background:rgba(0,0,0,.07);border-radius:4px">
                    <div style="height:5px;width:<?= $pctB ?>%;background:<?= $col ?>;border-radius:4px"></div>
                  </div>
                </div>
              <?php endforeach; ?>
            </div>
  ...
  <div id="cropBannerModal"
    ...
    <div
      style="background:#fff;border-radius:18px;padding:22px;max-width:680px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.5)">
      <div style="font-size:15px;font-weight:800;color:var(--brand);margin-bottom:12px;text-align:center">🖼️ Encuadra
        el banner</div>
      <div
        style="position:relative;width:100%;height:200px;overflow:hidden;border-radius:10px;background:#000;display:flex;align-items:center;justify-content:center">
      ...
      <div style="display:flex;gap:9px;margin-top:4px">
        <button onclick="cancelarCropBanner()"
          style="flex:1;padding:10px;border-radius:9px;border:1px solid #e0e0e0;background:#f5f5f5;font-size:12.5px;font-weight:700;cursor:pointer">Cancelar</button>
        <button onclick="confirmarCropBanner()" id="btnConfirmarCropBanner" class="btn-primary"
          style="flex:2;justify-content:center">✅ Usar banner</button>
      </div>
    </div>
  </div>
  ...
  <div class="modal-ov" id="modalGaleria">
    <div class="modal-box">
      ...
      <div class="modal-pad">
        ...
        <div class="mfila">
          <div class="mgr full"><label>Título de la foto</label><input type="text" id="galTitulo"
              placeholder="ej: Interior del local, Menú del día…"></div>
          <div class="mgr full"><label>Descripción (opcional)</label><textarea id="galDesc" rows="2"
              placeholder="Descripción breve…"></textarea></div>
        </div>
        <div
          style="border:2px dashed var(--brand-mid);border-radius:12px;padding:24px;text-align:center;cursor:pointer;background:var(--brand-light);transition:.2s"
          onclick="document.getElementById('galFile').click()" onmouseover="this.style.background='#fef3c7'"
          onmouseout="this.style.background='var(--brand-light)'">
        ...
        <input type="file" id="galFile" accept="image/jpeg,image/png,image/webp" style="display:none"
          onchange="subirEvidencia(this)">
        <div id="galProgress"
          style="display:none;margin-top:12px;font-size:12.5px;color:var(--brand);font-weight:600;text-align:center">⏳
          Subiendo…</div>
      </div>
    </div>
  </div>
    ...
    <div class="modal-box">
      ...
      <div class="modal-pad">
        ...
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">
          <div id="fotoPreview"
            style="width:68px;height:68px;border-radius:12px;background:linear-gradient(135deg,var(--brand),var(--brand2));display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:900;color:white;overflow:hidden;flex-shrink:0;cursor:pointer"
            onclick="document.getElementById('fotoInput').click()">
            <?php if ($fotoUrl): ?><img src="<?= $fotoUrl ?>" id="fotoImgPreview"
                style="width:100%;height:100%;object-fit:cover;border-radius:12px"><?php else: ?><span><?= $iniciales ?></span><?php endif; ?>
          </div>
          <div>
            ...
            <div style="display:flex;gap:7px;flex-wrap:wrap;align-items:center">
              <button onclick="document.getElementById('fotoInput').click()" class="btn-primary"
                style="font-size:12px;padding:7px 13px">📷 Cambiar foto</button>
              <?php if ($fotoUrl): ?><button onclick="eliminarFoto()" id="btnEliminarFoto"
                  style="padding:7px 13px;border-radius:8px;background:transparent;color:#e74c3c;border:1.5px solid #e74c3c;font-size:12.5px;font-weight:700;cursor:pointer">🗑
                  Quitar</button><?php endif; ?>
            </div>
        ...
        <div class="mfila">
          <div class="mgr full"><label>Nombre del negocio *</label><input type="text" id="editNombre"
              value="<?= htmlspecialchars($np['nombre_negocio'] ?? $usuario['nombre'] ?? '') ?>"></div>
        </div>
        <div class="mfila">
          ...
          <div class="mgr"><label>Ciudad</label><input type="text" id="editCiudad"
              value="<?= htmlspecialchars($uRe['ciudad'] ?? '') ?>" placeholder="Quibdó"></div>
        </div>
        <div class="mfila">
          <div class="mgr"><label>Dirección</label><input type="text" id="editDireccion"
              value="<?= htmlspecialchars($np['direccion'] ?? '') ?>" placeholder="Calle 27 # 4-15"></div>
          <div class="mgr"><label>Barrio</label><input type="text" id="editBarrio"
              value="<?= htmlspecialchars($np['barrio'] ?? '') ?>" placeholder="ej: Alameda Reyes"></div>
        </div>
        <div class="mfila">
          <div class="mgr"><label>WhatsApp</label><input type="tel" id="editWhatsapp"
              value="<?= htmlspecialchars($np['whatsapp'] ?? '') ?>" placeholder="3001234567"></div>
          <div class="mgr"><label>Horario de atención</label><input type="text" id="editHorario"
              value="<?= htmlspecialchars($np['horario'] ?? '') ?>" placeholder="Lun-Sab 8am-8pm"></div>
        </div>
        <div class="mfila">
          <div class="mgr"><label>Instagram</label><input type="text" id="editInstagram"
              value="<?= htmlspecialchars($np['instagram'] ?? '') ?>" placeholder="@minegocio"></div>
          <div class="mgr"><label>Sitio web</label><input type="url" id="editSitioWeb"
              value="<?= htmlspecialchars($np['sitio_web'] ?? '') ?>" placeholder="https://…"></div>
        </div>
        <div class="mfila">
          <div class="mgr full"><label>Descripción del negocio</label><textarea id="editDescripcion" rows="3"
              placeholder="¿Qué ofreces? ¿Qué hace especial a tu negocio?"><?= htmlspecialchars($np['descripcion'] ?? '') ?></textarea>
          </div>
        </div>
        ...
        <div style="margin-top:28px;padding-top:20px;border-top:1px solid rgba(239,68,68,.2)">
          <div style="display:flex;align-items:center;gap:7px;margin-bottom:9px"><span>⚠️</span><span
              style="font-size:12px;font-weight:700;color:#f87171;text-transform:uppercase;letter-spacing:.8px">Zona de
              peligro</span></div>
          <p style="font-size:12.5px;color:var(--ink3);margin-bottom:12px;line-height:1.5">Eliminar tu cuenta es <strong
              style="color:#f87171">permanente e irreversible</strong>.</p>
          <button onclick="abrirEliminarCuenta()"
            style="padding:9px 18px;border-radius:9px;background:transparent;border:1.5px solid #e74c3c;color:#e74c3c;font-size:12.5px;font-weight:700;cursor:pointer;font-family:inherit;transition:.2s"
  ...
  <div id="modalEliminarCuenta"
    ...
    <div
      style="background:#0f172a;border:1.5px solid rgba(239,68,68,.35);border-radius:18px;padding:32px 28px;max-width:420px;width:100%;box-shadow:0 24px 64px rgba(0,0,0,.6)">
      <div style="text-align:center;margin-bottom:20px">
        ...
        <p style="font-size:13px;color:rgba(255,255,255,.6);line-height:1.6">Se eliminarán el negocio, galería, mensajes
          y todos los datos asociados.</p>
      </div>
      <div style="margin-bottom:18px">
        <label
          style="display:block;font-size:12.5px;font-weight:600;color:rgba(255,255,255,.7);margin-bottom:7px">Escribe tu
          correo: <strong style="color:#f87171"><?= $correo ?></strong></label>
        <input type="text" id="inputConfirmarCuenta" placeholder="Escribe tu correo exacto"
          style="width:100%;padding:10px 13px;border-radius:9px;border:1.5px solid rgba(239,68,68,.4);background:rgba(239,68,68,.06);color:white;font-size:13px;outline:none;font-family:inherit">
      </div>
      <div id="msgEliminarCuenta"
        style="display:none;margin-bottom:10px;padding:9px 13px;border-radius:8px;font-size:12.5px"></div>
      <div style="display:flex;gap:9px">
        <button onclick="cerrarEliminarCuenta()"
          style="flex:1;padding:11px;border-radius:9px;border:1.5px solid rgba(255,255,255,.15);background:transparent;color:rgba(255,255,255,.7);font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Cancelar</button>
        <button id="btnConfirmarEliminar" onclick="confirmarEliminarCuenta()"
          style="flex:1;padding:11px;border-radius:9px;border:none;background:#e74c3c;color:white;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">🗑
          Sí, eliminar</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    async function toggleVisibilidad(visible) {
      ...
      const togInput = document.querySelector('.tog input[type=checkbox]');
      ...
      try {
        ...
        if (j.ok) {
          ...
          if (togInput) togInput.checked = !visible;
          chip.textContent = '🟡 Oculto';
          chip.className = 'pv-chip off';
          if (j.requiere_plan) {
            alert('⚠️ ' + j.msg + '\n\nMejora tu plan desde el directorio de empresas.');
          } else {
            alert('⚠️ ' + (j.msg || 'No se pudo cambiar la visibilidad.'));
          }
        }
      } catch (e) { }
    }
    ...
    async function subirFoto(input) {
      ...
      try {
        const r = await fetch('dashboard_negocios.php', { method: 'POST', body: fd }); const j = await r.json();
        if (j.ok) {
          msg.textContent = '✅ Foto actualizada'; msg.style.color = 'var(--brand)';
          const imgTag = `<img src="${j.foto}?t=${Date.now()}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
          const imgTagSq = `<img src="${j.foto}?t=${Date.now()}" style="width:100%;height:100%;object-fit:cover;border-radius:12px">`;
          ['heroAvatar', 'sidebarAvatar', 'dashAvatarImg', 'dropAv'].forEach(id => { const el = document.getElementById(id); if (el) el.innerHTML = imgTag; });
          ['fotoPreview', 'cpAvatarCard', 'logoAvCard'].forEach(id => { const el = document.getElementById(id); if (el) el.innerHTML = imgTagSq; });
    ...
    async function cargarNotificaciones() {
      try {
        const r = await fetch('api_usuario.php?_action=notificaciones'); const j = await r.json();
