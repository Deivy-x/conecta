...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'subir_logo') {
    ...
    if ($_FILES['logo']['size'] > 2 * 1024 * 1024) {
      echo json_encode(['ok' => false, 'msg' => 'Imagen demasiado grande (máx 2 MB).']);
      exit;
    }
    ...
    if ($oldLogo && file_exists($dir . $oldLogo))
      @unlink($dir . $oldLogo);
  ...
  if ($action === 'publicar_vacante') {
    ...
    if (!$titulo || !$tipo || !$descripcion || !$requisitos) {
      echo json_encode(['ok' => false, 'msg' => 'Faltan campos obligatorios (título, tipo, descripción y requisitos).']);
      exit;
    }
...
$ep = $epStmt->fetch() ?: [
  ...
  'telefono_empresa' => '',
  'municipio' => '',
  'avatar_color' => 'linear-gradient(135deg,#1a56db,#3b82f6)',
  'visible' => 1,
  'visible_admin' => 1,
  'destacado' => 0,
  'vacantes_activas' => 0
...
<head>
  ...
  <style>
    :root {
      ...
      --arena: #f9f3e3;
      --borde: rgba(0, 0, 0, .09);
      --sombra: 0 2px 16px rgba(0, 0, 0, .07)
    }
    ...
    *::after {
    ...
    /* ── HERO ── */
    .hero {
      background: linear-gradient(135deg, #0a1a3e 0%, #1a3060 60%, #0a1a3e 100%);
    ...
    .hero::after {
      ...
      position: absolute;
      inset: 0;
      background: radial-gradient(ellipse at 80% 50%, rgba(26, 86, 219, .15) 0%, transparent 60%);
      pointer-events: none
    }
    ...
    .ac:hover {
      ...
      box-shadow: 0 6px 16px rgba(26, 86, 219, .1)
    }
  ...
  <div style="padding:24px 40px 0">
    <?php if (!$tieneVerificado): ?>
      ...
      <?php elseif ($estadoVerif === 'rechazado'): ?>
        <div class="alerta ar">
          <div class="a-ico">❌</div>
          <div class="a-txt"><strong>Verificación rechazada</strong><span>Intenta subir los documentos con mejor
              calidad.</span></div>
        ...
        <div class="hist-list" id="historialVacantes">
            ...
            <?php foreach ($historialVacantes as $v): ?>
              <div class="hist-item">
                ...
                <div class="hist-info">
                  ...
                  <div class="hist-meta">📍 <?= htmlspecialchars($v['ciudad'] ?? 'Chocó') ?> ·
                    <?= htmlspecialchars($v['tipo_contrato'] ?? '') ?> ·
                    <?= $v['activo'] ? '<span style="color:#22c55e;font-weight:700">Activa</span>' : '<span style="color:#94a3b8">Cerrada</span>' ?>
                  </div>
                </div>
          ...
          <?php else: ?>
            <div style="text-align:center;padding:32px 20px;color:var(--ink3)">
              ...
              <div style="font-size:14px;font-weight:700;color:var(--ink2);margin-bottom:6px">Aún no has publicado
                vacantes</div>
              <div style="font-size:13px">Conecta con talentos del Chocó publicando tu primera oferta</div>
              <a href="javascript:void(0)" onclick="abrirModalVacante()"
                style="display:inline-block;margin-top:16px;padding:10px 22px;background:var(--azul);color:white;border-radius:10px;text-decoration:none;font-weight:800;font-size:13px">➕
      ...
      <div class="card" style="display:flex;flex-direction:column">
        ...
        <div class="cp-body">
          ...
          <?php if (!empty($ep['sitio_web'])): ?>
            <div class="cp-fil"><span class="cp-ico">🌐</span><a href="<?= htmlspecialchars($ep['sitio_web']) ?>"
                target="_blank"
                style="color:var(--azul);font-size:13px;text-decoration:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= htmlspecialchars($ep['sitio_web']) ?></a>
            </div>
          <?php endif; ?>
        ...
        <div class="badge-row">
          ...
          <?php if (!$tieneVerificado && !$tienePremium && !$tieneDestacado && !$tieneTop): ?>
            <div style="font-size:13px;color:var(--ink3);padding:8px 0">
              Aún no tienes badges. <a href="verificar_cuenta.php" style="color:var(--azul);font-weight:700">Verifica tu
                empresa →</a>
            </div>
          <?php endif; ?>
        </div>
        <div
          style="margin-top:16px;padding:14px;background:var(--azul-claro);border-radius:12px;border:1px solid #bfdbfe">
          <div style="font-size:12px;font-weight:800;color:var(--azul);margin-bottom:6px">¿Quieres más visibilidad?
          </div>
          <div style="font-size:12px;color:#1e40af;line-height:1.5">Los badges <strong>Premium</strong> y
            <strong>Destacada</strong> son asignados por el administrador. Contacta al equipo para más información.
          </div>
        </div>
      </div>
    ...
    <div class="modal-box">
      ...
      <div class="modal-pad">
        ...
        <div style="display:flex;align-items:center;gap:16px;margin-bottom:18px">
          <div id="logoPreview"
            style="width:72px;height:72px;border-radius:14px;background:linear-gradient(135deg,var(--azul),var(--azul2));display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:900;color:white;overflow:hidden;flex-shrink:0;cursor:pointer;border:3px solid rgba(26,86,219,.2)"
            onclick="document.getElementById('logoInput').click()">
            <?php if ($logoUrl): ?>
              <img src="<?= $logoUrl ?>" id="logoImgPreview"
          ...
          <div>
            ...
            <button onclick="document.getElementById('logoInput').click()"
              style="padding:8px 14px;border-radius:8px;background:var(--azul);color:white;border:none;font-size:13px;font-weight:700;cursor:pointer">🖼️
              Cambiar logo</button>
          ...
          <div class="mfila">
            <div class="mgr full">
              ...
              <textarea name="descripcion" rows="3"
                placeholder="¿Qué hará esta persona en el día a día? ¿Qué ofrece tu empresa?" required></textarea>
            </div>
          </div>
          <div class="mfila">
            <div class="mgr full">
              ...
              <textarea name="requisitos" rows="3"
                placeholder="ej: Experiencia mínima 1 año · Enviar hoja de vida a rrhh@empresa.com" required></textarea>
            </div>
          </div>
